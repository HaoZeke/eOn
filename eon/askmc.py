...
processtable_header = processtable_head_fmt % (
    "proc #",
    "saddle energy",
    "prefactor",
    "product",
...
mod_processtable_header = processtable_head_fmt % (
    "proc #",
    "saddle energy",
    "prefactor",
    "product",
...
class ASKMC:
    ...
    def compile_process_table(self, current_state):
        ...
        for process_id in list(current_state_mod_procs.keys()):
            try:
                current_state_procs_compiled[process_id] = current_state_mod_procs[
                    process_id
                ]
            except KeyError:
                ...
                raise Exception(
                    "Somehow the modified process table has a process that\
    ...
    def append_modified_process_table(
        self,
        current_state,
        process_id,
        saddle_energy,
    ...
    def register_transition(self, current_state, next_state):
        ...
        next_state_process_id = self.get_process_id(
            current_state_mod_procs, next_state.number, "try"
        )
        ...
        if next_state_process_id is None:
            ...
            next_state_process_id = self.get_process_id(
                current_state_real_procs, next_state.number, "find"
            )
            ...
            rate = procs["rate"]
            self.append_modified_process_table(
                current_state,
                next_state_process_id,
                saddle_energy,
                prefactor,
                product,
                product_energy,
        ...
        # Otherwise it's been seen before, and one simply needs to be added to the view_count
            ...
            if (
                current_state_mod_procs[next_state_process_id]["view_count"]
                >= (2**sb_check_count) * self.Nf
            ):
                self.raiseup(
                    current_state, next_state, sb_check_count, num_rate_changes
                )
    ...
    def missed_connections(self, origEtrans):
        ...
        states_in_basin = [
            self.states.get_state(state_num) for state_num in state_nums_in_basin
        ]
        ...
        for state in states_in_basin:
            ...
            for process_id in list(state_real_procs.keys()):
                ...
                if (
                    product_state_num in state_nums_in_basin
                    and not (
                        self.in_array([product_state_num, state.number], self.edgelist)
                        or self.in_array(
                            [state.number, product_state_num], self.edgelist
                        )
                    )
                    and state_real_procs[process_id]["saddle_energy"]
                    < origEtrans + math.log(self.gamma) / self.Beta
                ):
                    testvar = 1
                    return testvar
        return testvar
    ...
    def missed_low_barriers(self, origEtrans):
        ...
        for state in states_in_basin:
            ...
            for process_id in list(state_real_procs.keys()):
                if (
                    process_id not in mod_proc_ids
                    and state_real_procs[process_id]["saddle_energy"]
                    < origEtrans + math.log(self.gamma) / self.Beta
                ):
                    testvar = 1
            ...
            for next_state_id in list(current_state_mod_procs.keys()):
                ...
                if self.welltest == 1:
                    ...
                    if (
                        not (
                            self.in_array(
                                [current_state.number, next_state_num], self.edgelist
                            )
                            or self.in_array(
                                [next_state_num, current_state.number], self.edgelist
                            )
                        )
                        and current_state_mod_procs[next_state_id]["saddle_energy"]
                        < origEtrans + math.log(self.gamma) / self.Beta
                    ):
                        next_state = self.states.get_state(next_state_num)
                        next_state_mod_procs = self.get_modified_process_table(
                            next_state
                        )
                        # Try to find the "process_id" of the current state relative to the next state
                        current_state_id = self.get_process_id(
                            next_state_mod_procs, current_state.number, "try"
                        )
                        ...
                        if (
                            current_state_mod_procs[next_state_id]["view_count"]
                            < self.Nf
                            or current_state_id is None
                            or next_state_mod_procs[current_state_id]["view_count"]
                            < self.Nf
                        ):
                            self.welltest = 0
                        # Otherwise, add this process to "edgelist" to note it's been viewed, and continue the search
                        else:
                            self.edgelist = numpy.vstack(
                                (self.edgelist, [current_state.number, next_state_num])
                            )
    ...
    def raiseup(self, current_state, next_state, sb_check_count, num_rate_changes):
        ...
        next_state_id = self.get_process_id(
            current_state_mod_procs, next_state.number, "find"
        )
        ...
        if self.welltest:
            ...
            if self.checksb_connections and self.missed_connections(origEtrans):
                ...
                continue_flag = 0
                logger.info(
                    "Based on 'connections-check', default superbasin criterion apparently missed a barrier - Not raising the barriers"
                )
            ...
            if self.checksb_barriers and self.missed_low_barriers(origEtrans):
                continue_flag = 0
                logger.info(
                    "Based on 'Barrier-check', default superbasin criterion apparently missed a barrier - Not raising the barriers"
                )
            if continue_flag:
                ...
                for state_pair in self.edgelist:
                    ...
                    logger.info(
                        "Raising barrier between states %d and %d"
                        % (state_a_num, state_b_num)
                    )
                    # The state objects
                    ...
                    state_b_id = self.get_process_id(
                        state_a_mod_procs, state_b_num, "find"
                    )
                    state_a_id = self.get_process_id(
                        state_b_mod_procs, state_a_num, "find"
                    )
                    # The new rates
                    ...
                    a_b_new_saddle = (
                        math.log(
                            a_b_new_rate / state_a_mod_procs[state_b_id]["prefactor"]
                        )
                        / (-self.Beta)
                        + state_a_energy
                    )
                    b_a_new_saddle = (
                        math.log(
                            b_a_new_rate / state_b_mod_procs[state_a_id]["prefactor"]
                        )
                        / (-self.Beta)
                        + state_b_energy
                    )
                    if not self.is_equal(a_b_new_saddle, b_a_new_saddle):
                        ...
                        logger.warning(
                            "When recalculating saddle energies from changed rate constants,\
                ...
                if self.sb_recycling_on:
                    ...
                    sb_data_path = recycle_path / "current_sb_states"
                    state_list = self.edgelist_to_statelist()
                    sb_data_path.write_text(
