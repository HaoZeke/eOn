...
def make_movie(movie_type, path_root, states, separate_files=False):
    ...
    if separate_files:
        ...
        movie_path = movie_dir / movie_path.name
        for old_poscar in movie_dir.glob(movie_path.name + ".*"):
            old_poscar.unlink()
        for i, atoms in enumerate(atoms_list):
            path_i = movie_dir / (movie_path.name + (".%010d" % i))
            io.saveposcar(str(path_i), atoms, "w")
        print("Saved %i frames to %s.*" % (len(atoms_list), movie_path))
    else:
        if movie_path.is_file():
            movie_path.unlink()
        for atoms in atoms_list:
            io.saveposcar(str(movie_path), atoms, "a")
        print("Saved %i frames to %s" % (len(atoms_list), movie_path))
...
def processes(states, statenr, limit):
    ...
    for p in sorted_processes:
        ...
        print(
            "%4i %16.5e %16.5f %16.5e"
            % (p["id"], p["rate"], p["barrier"], p["prefactor"])
        )
        limit -= 1
        if limit == 0:
            break
...
def dynamics(path_root, states, unique=False):
    if not unique:
        trajectory_path = Path(path_root) / "dynamics.txt"
        trajectory = get_trajectory(str(trajectory_path))
    else:
...
def fastest_path(path_root, states, full=False):
    ...
    for i in range(len(state_list)):
        ...
        if len(state_list) - 1 == i:
            continue
        atoms_list.append(
            state_list[i].get_process_saddle(
                get_fastest_process_id(state_list[i], state_list[i + 1])
            )
        )
    time = 0.0
