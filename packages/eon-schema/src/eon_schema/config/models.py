"""L1 full job-config pydantic models (shared by eon-akmc + docs).

Authoritative home: ``eon_schema.config.models``. The eon-akmc module
``eon.schema`` re-exports this surface so Sphinx ``autopydantic_model``
targets and existing imports keep working.

For Main, Potential, Optimizer, Structure Comparison, and Process Search,
field names must match schema/eon_params.capnp (Cap'n Proto L0 SSoT).
...
class RgpotPot(BaseModel):
    ...
    title: str = Field(default="", description="Job title passed to the engine.")
    memory_mb: int = Field(
        default=0, description="Engine memory limit in MB (0 uses the engine default)."
    )
