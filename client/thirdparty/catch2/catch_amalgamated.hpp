...
// still have the compiler warn on problems inside...
    ...
    !defined(CATCH_INTERNAL_CONFIG_NO_USE_BUILTIN_CONSTANT_P) &&               \
    !defined(CATCH_CONFIG_USE_BUILTIN_CONSTANT_P)
...
public:
  ...
  virtual IGeneratorTracker *
  createGeneratorTracker(StringRef generatorName, SourceLineInfo lineInfo,
...
public:
  ...
  virtual void
  registerListener(Detail::unique_ptr<EventListenerFactory> factory) = 0;
  virtual void registerTest(Detail::unique_ptr<TestCaseInfo> &&testInfo,
                            Detail::unique_ptr<ITestInvoker> &&invoker) = 0;
  virtual void
  registerTranslator(Detail::unique_ptr<IExceptionTranslator> &&translator) = 0;
...
static constexpr bool is_related_v =
...
struct ExecutionPlan {
  ...
  std::vector<FDuration> run(const IConfig &cfg, Environment env) const {
    ...
    Detail::run_for_at_least<Clock>(
        std::chrono::duration_cast<IDuration>(warmup_time), warmup_iterations,
...
EnvironmentEstimate estimate_clock_resolution(int iterations) {
  auto r = run_for_at_least<Clock>(clock_resolution_estimation_time, iterations,
                                   &resolution<Clock>)
               .result;
  return {
...
EnvironmentEstimate estimate_clock_cost(FDuration resolution) {
  ...
  for (int s = 0; s < nsamples; ++s) {
    times.push_back(
        static_cast<double>((time_clock(r.iterations) / r.iterations).count()));
  }
  return {
...
struct Benchmark {
  ...
  template <typename Clock = default_clock> void run() {
    ...
    CATCH_TRY {
      ...
      auto analysis = Detail::analyse(*cfg, samples.data(),
                                      samples.data() + samples.size());
      BenchmarkStats<> stats{CATCH_MOVE(info),  CATCH_MOVE(analysis.samples),
  ...
  template <typename Fun,
            std::enable_if_t<!Detail::is_related_v<Fun, Benchmark>, int> = 0>
...
static constexpr bool IsStreamInsertable_v<
    T, decltype(void(std::declval<std::ostream &>() << std::declval<T>()))> =
    true;
...
template <typename T, typename = void> struct StringMaker {
  ...
  static std::enable_if_t<::Catch::Detail::IsStreamInsertable_v<Fake>,
                          std::string>
...
public:
  ReporterSpec(std::string name, Optional<std::string> outputFileName,
...
struct MessageInfo {
  MessageInfo(StringRef _macroName, SourceLineInfo const &_lineInfo,
...
struct MessageBuilder : MessageStream {
  MessageBuilder(StringRef macroName, SourceLineInfo const &lineInfo,
                 ResultWas::OfType type)
...
#define INTERNAL_CATCH_CAPTURE(varName, macroName, ...)                        \
  Catch::Capturer varName(macroName##_catch_sr, CATCH_INTERNAL_LINEINFO,       \
                          Catch::ResultWas::Info, #__VA_ARGS__##_catch_sr);    \
...
static constexpr bool is_unary_function_v<
    F, Catch::Detail::void_t<decltype(std::declval<F>()(fake_arg()))>> = true;
...
public:
  ...
  template <typename LambdaT, typename = typename std::enable_if_t<
...
public:
  ...
  Detail::InternalParseResult parse(std::string const &,
...
public:
  ...
  using ParserBase::parse;
  Detail::InternalParseResult parse(std::string const &exeName,
                                    Detail::TokenStream tokens) const override;
};
...
#define CATCH_REGISTER_TAG_ALIAS(alias, spec)                                  \
  ...
  namespace {                                                                  \
  const Catch::RegistrarForTagAliases INTERNAL_CATCH_UNIQUE_NAME(              \
...
#define CATCH_DEFINE_COMPARABLE_TRAIT(id, op)                                  \
  ...
  struct is_##id##_comparable<                                                 \
      T, U, void_t<decltype(std::declval<T>() op std::declval<U>())>>          \
      : std::true_type {};                                                     \
...
public:
  ...
  constexpr ITransientExpression &
  operator=(ITransientExpression const &) = default;
...
struct Decomposer {
  template <typename T,
            std::enable_if_t<!capture_by_value<Detail::RemoveCVRef_t<T>>::value,
                             int> = 0>
  constexpr friend auto operator<=(Decomposer &&, T &&lhs)
      -> ExprLhs<T const &> {
...
public:
  ...
  Section(SourceLineInfo const &_lineInfo, StringRef _name,
          const char *const = nullptr);
  ~Section();
...
makeTestInvokerFixture(void (C::*testAsMethod)() const) {
...
#define INTERNAL_CATCH_TESTCASE(...)                                           \
  INTERNAL_CATCH_TESTCASE2(INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEST_),  \
                           __VA_ARGS__)
...
#define INTERNAL_CATCH_TEST_CASE_METHOD2(TestName, ClassName, ...)             \
  ...
  const Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(              \
      ...
      #ClassName##_catch_sr, Catch::NameAndTags{__VA_ARGS__}); /* NOLINT */    \
  }                                                                            \
  CATCH_INTERNAL_STOP_WARNINGS_SUPPRESSION                                     \
  void TestName::test()
...
#define INTERNAL_CATCH_TEST_CASE_PERSISTENT_FIXTURE2(TestName, ClassName, ...) \
  ...
  const Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(              \
      Catch::makeTestInvokerFixture(&TestName::test), CATCH_INTERNAL_LINEINFO, \
      #ClassName##_catch_sr, Catch::NameAndTags{__VA_ARGS__}); /* NOLINT */    \
  }                                                                            \
  CATCH_INTERNAL_STOP_WARNINGS_SUPPRESSION                                     \
...
#define INTERNAL_CATCH_METHOD_AS_TEST_CASE(QualifiedMethod, ...)               \
  ...
  const Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(              \
      ...
      "&" #QualifiedMethod##_catch_sr, Catch::NameAndTags{__VA_ARGS__});       \
  } /* NOLINT */                                                               \
  CATCH_INTERNAL_STOP_WARNINGS_SUPPRESSION
...
#define CATCH_SKIP(...)                                                        \
  do {                                                                         \
    INTERNAL_CATCH_MSG("CATCH_SKIP", Catch::ResultWas::ExplicitSkip,           \
                       Catch::ResultDisposition::Normal, __VA_ARGS__);         \
    Catch::Detail::Unreachable();                                              \
  } while (false)
...
#define SKIP(...)                                                              \
  do {                                                                         \
    INTERNAL_CATCH_MSG("SKIP", Catch::ResultWas::ExplicitSkip,                 \
                       Catch::ResultDisposition::Normal, __VA_ARGS__);         \
    Catch::Detail::Unreachable();                                              \
  } while (false)
...
#define INTERNAL_CATCH_TYPE_GEN                                                \
  ...
  constexpr auto get_wrapper(Catch::Detail::priority_tag<1>) noexcept          \
      -> TypeList<Ts...> {                                                     \
    return {};                                                                 \
  }                                                                            \
...
#define INTERNAL_CATCH_NTTP_1(signature, ...)                                  \
  ...
  constexpr auto get_wrapper(Catch::Detail::priority_tag<0>) noexcept          \
      -> Nttp<__VA_ARGS__> {                                                   \
    return {};                                                                 \
  }                                                                            \
...
#define INTERNAL_CATCH_TYPES_REGISTER(TestFunc)                                \
  ...
  void reg_test(TypeList<Ts...>, Catch::NameAndTags nameAndTags) {             \
    Catch::AutoReg(Catch::makeTestInvoker(&TestFunc<Ts...>),                   \
...
#define INTERNAL_CATCH_NTTP_REG_GEN(TestFunc, ...)                             \
  INTERNAL_CATCH_TYPES_REGISTER(TestFunc)                                      \
  INTERNAL_CATCH_VA_NARGS_IMPL(                                                \
...
#define INTERNAL_CATCH_TEMPLATE_TEST_CASE_NO_REGISTRATION(Name, Tags, ...)     \
  INTERNAL_CATCH_TEMPLATE_TEST_CASE_NO_REGISTRATION_2(                         \
      ...
      INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEMPLATE_TEST_), Name, Tags,  \
...
#define INTERNAL_CATCH_TEMPLATE_TEST_CASE_NO_REGISTRATION(Name, Tags, ...)     \
  INTERNAL_CATCH_EXPAND_VARGS(                                                 \
      INTERNAL_CATCH_TEMPLATE_TEST_CASE_NO_REGISTRATION_2(                     \
          INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEMPLATE_TEST_),          \
          INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEMPLATE_TEST_), Name,    \
...
#define INTERNAL_CATCH_TEMPLATE_TEST_CASE_METHOD_NO_REGISTRATION(              \
  ...
  INTERNAL_CATCH_TEMPLATE_TEST_CASE_METHOD_NO_REGISTRATION_2(                  \
      ...
      INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEMPLATE_TEST_), ClassName,   \
    ...
    void reg_tests() {                                                         \
      ...
      (void)expander{                                                          \
          (Catch::AutoReg(                                                     \
               ...
               CATCH_INTERNAL_LINEINFO, #ClassName##_catch_sr,                 \
               Catch::NameAndTags{                                             \
                   Name " - " INTERNAL_CATCH_STRINGIZE(TmplList) " - " +       \
                       std::to_string(index),                                  \
                   Tags}),                                                     \
...
struct TestCaseInfo : Detail::NonCopyable {
  ...
  TestCaseInfo(StringRef _className, NameAndTags const &_nameAndTags,
...
makeTestCaseInfo(StringRef className, NameAndTags const &nameAndTags,
                 SourceLineInfo const &lineInfo);
} // namespace Catch
...
#define INTERNAL_CATCH_TRANSLATE_EXCEPTION2(translatorName, signature)         \
  ...
  CATCH_INTERNAL_SUPPRESS_GLOBALS_WARNINGS                                     \
  namespace {                                                                  \
  const Catch::ExceptionTranslatorRegistrar INTERNAL_CATCH_UNIQUE_NAME(        \
...
template <typename IntegerType> class uniform_integer_distribution {
  ...
  static constexpr UnsignedIntegerType
  computeRejectionThreshold(UnsignedIntegerType ab_distance) {
...
DistanceType<FloatType> count_equidistant_floats(FloatType a, FloatType b,
                                                 FloatType distance) {
  assert(a <= b);
...
template <typename InputIterator, typename InputSentinel,
          typename ResultType = std::remove_const_t<
...
struct ReporterConfig {
  ReporterConfig(IConfig const *_fullConfig,
                 Detail::unique_ptr<IStream> _stream, ColourMode colourMode,
                 std::map<std::string, std::string> customOptions);
...
struct SectionStats {
  SectionStats(SectionInfo &&_sectionInfo, Counts const &_assertions,
...
public:
  ...
  //! format
  virtual void
  listReporters(std::vector<ReporterDescription> const &descriptions) = 0;
  //! Writes out the provided listeners descriptions using reporter-specific
  //! format
  virtual void
  listListeners(std::vector<ListenerDescription> const &descriptions) = 0;
...
class EnumValuesRegistry : public IMutableEnumValuesRegistry {
  ...
  EnumInfo const &registerEnum(StringRef enumName, StringRef allValueNames,
...
public:
  ...
  void
  registerTranslator(Detail::unique_ptr<IExceptionTranslator> &&translator);
...
template <typename ForwardIter, typename Sentinel, typename T,
          typename Comparator>
constexpr ForwardIter find_sentinel(ForwardIter start, Sentinel sentinel,
                                    T const &value, Comparator cmp) {
...
template <typename ForwardIter, typename Sentinel, typename T,
          typename Comparator>
constexpr std::ptrdiff_t count_sentinel(ForwardIter start, Sentinel sentinel,
                                        T const &value, Comparator cmp) {
...
template <typename ForwardIter1, typename Sentinel1, typename ForwardIter2,
          typename Sentinel2, typename Comparator>
constexpr bool check_element_counts(ForwardIter1 first_1, const Sentinel1 end_1,
                                    ForwardIter2 first_2, const Sentinel2 end_2,
                                    Comparator cmp) {
  auto cursor = first_1;
  while (cursor != end_1) {
...
template <typename ForwardIter1, typename Sentinel1, typename ForwardIter2,
          typename Sentinel2, typename Comparator>
constexpr bool is_permutation(ForwardIter1 first_1, const Sentinel1 end_1,
                              ForwardIter2 first_2, const Sentinel2 end_2,
                              Comparator cmp) {
  // TODO: no optimization for stronger iterators, because we would also have to
  // constrain on sentinel vs not sentinel types
...
struct JsonUtils {
  ...
  static void appendCommaNewline(std::ostream &os, bool &should_comma,
                                 std::uint64_t level);
};
...
public:
  ...
  // TODO: Do we want to check that redirect is not active before retrieving the
  // output?
...
public:
  ...
  std::map<std::string, IReporterFactoryPtr,
...
public:
  TrackerBase(NameAndLocation &&nameAndLocation, TrackerContext &ctx,
              ITracker *parent);
...
class SectionTracker : public TrackerBase {
  ...
  // Note that lifetime-wise we piggy back off the name stored in the `ITracker`
  // parent`. Currently it allocates owns the name, so this is safe. If it is
  // later refactored to not own the name, the name still has to outlive the
  // `ITracker` parent, so this should still be safe.
  StringRef m_trimmed_name;

public:
  SectionTracker(NameAndLocation &&nameAndLocation, TrackerContext &ctx,
                 ITracker *parent);
...
private:
  ...
  // Caches whether `assertionEnded` events for successful assertions should be
...
template <typename Container>
Container createShard(Container const &container, std::size_t const shardCount,
...
public:
  ...
  class const_iterator {
    ...
    std::string
    addIndentAndSuffix(AnsiSkippingString::const_iterator start,
...
template <typename... Ts>
static constexpr bool are_generic_matchers_v = Catch::Detail::conjunction<
    std::integral_constant<bool, is_generic_matcher_v<Ts>>...>::value;
...
public:
  ...
  operator||(MatchAnyOfGeneric<MatcherTs...> &&lhs, MatcherRHS const &rhs) {
    return MatchAnyOfGeneric<MatcherTs..., MatcherRHS>{
        array_cat(CATCH_MOVE(lhs.m_matchers),
...
//! matcher
template <typename Matcher>
std::enable_if_t<Detail::is_matcher_v<Matcher>, ContainsMatcherMatcher<Matcher>>
...
public:
  ...
  template <typename RangeLike> constexpr bool match(RangeLike &&rng) const {
    using std::begin;
    using std::end;
    return Catch::Detail::is_permutation(begin(m_desired), end(m_desired),
                                         begin(rng), end(rng), m_predicate);
  }
...
public:
   ...
   */
  void
  listReporters(std::vector<ReporterDescription> const &descriptions) override;
  /**
   ...
   */
  void
  listListeners(std::vector<ListenerDescription> const &descriptions) override;
  /**
...
#define CATCH_REGISTER_REPORTER(name, reporterType)                            \
  ...
  const Catch::ReporterRegistrar<reporterType>                                 \
      INTERNAL_CATCH_UNIQUE_NAME(catch_internal_RegistrarFor)(name);           \
  }                                                                            \
  CATCH_INTERNAL_STOP_WARNINGS_SUPPRESSION
...
#define CATCH_REGISTER_LISTENER(listenerType)                                  \
  ...
  CATCH_INTERNAL_SUPPRESS_GLOBALS_WARNINGS                                     \
  namespace {                                                                  \
  const Catch::ListenerRegistrar<listenerType> INTERNAL_CATCH_UNIQUE_NAME(     \
