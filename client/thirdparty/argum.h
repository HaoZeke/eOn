...
#define ARGUM_DECLARE_FRIENDLY_NAME(stem, type, prefix)                        \
  ARGUM_MOD_EXPORTED using prefix##stem = Basic##stem<type>;
...
#else
#define ARGUM_ALWAYS_ASSERT(x)                                                 \
  ((void)((x) ? ((void)0)                                                      \
              : ::Argum::terminateApplication("failed assertion `" #x "'")))
#endif
...
concept StringLike = requires(X name) {
  ...
  requires std::is_convertible_v<
      X, std::basic_string_view<std::remove_cvref_t<decltype(name[0])>>>;
};

template <StringLike S>
using CharTypeOf = std::remove_cvref_t<decltype(std::declval<S>()[0])>;

template <class X, class Char>
concept StringLikeOf =
    Character<Char> && std::is_convertible_v<X, std::basic_string_view<Char>>;
...
template <Character Char> auto makeString(Char c) -> std::basic_string<Char> {
...
auto join(It1 first, It2 last, Joiner joiner, Projection proj)
    -> decltype(proj(*first) + joiner) {
...
template <class Char>
auto matchPrefix(std::basic_string_view<Char> value,
                 std::basic_string_view<Char> prefix) -> bool {
...
template <class Char>
auto matchStrictPrefix(std::basic_string_view<Char> value,
                       std::basic_string_view<Char> prefix) -> bool {
...
#define ARGUM_DEFINE_CHAR_CONSTANTS(type, prefix)                              \
  template <> struct CharConstants<type> {                                     \
    ...
    static constexpr auto mustEscapeInRegex =                                  \
        prefix##R"([.^$|()\[\]{}*+?\\])";                                      \
    static constexpr auto regexEscapeReplacement = prefix##R"(\\&)";           \
    static constexpr auto falseNames = {prefix##"0", prefix##"false",          \
                                        prefix##"off", prefix##"no"};          \
    static constexpr auto trueNames = {prefix##"1", prefix##"true",            \
                                       prefix##"on", prefix##"yes"};           \
                                                                               \
    static auto isSpace(type c) -> bool;                                       \
    static auto toLong(const type *str, type **str_end, int base) -> long;     \
    static auto toLongLong(const type *str, type **str_end, int base)          \
        -> long long;                                                          \
    static auto toULong(const type *str, type **str_end, int base)             \
        -> unsigned long;                                                      \
    static auto toULongLong(const type *str, type **str_end, int base)         \
        -> unsigned long long;                                                 \
...
inline auto CharConstants<wchar_t>::isSpace(wchar_t c) -> bool {
  return iswspace(c);
}

inline auto CharConstants<char>::toLong(const char *str, char **str_end,
                                        int base) -> long {
  return strtol(str, str_end, base);
}
inline auto CharConstants<wchar_t>::toLong(const wchar_t *str,
                                           wchar_t **str_end, int base)
    -> long {
  return wcstol(str, str_end, base);
}

inline auto CharConstants<char>::toLongLong(const char *str, char **str_end,
                                            int base) -> long long {
  return strtoll(str, str_end, base);
}
inline auto CharConstants<wchar_t>::toLongLong(const wchar_t *str,
                                               wchar_t **str_end, int base)
    -> long long {
  return wcstoll(str, str_end, base);
}

inline auto CharConstants<char>::toULong(const char *str, char **str_end,
                                         int base) -> unsigned long {
  return strtoul(str, str_end, base);
}
inline auto CharConstants<wchar_t>::toULong(const wchar_t *str,
                                            wchar_t **str_end, int base)
    -> unsigned long {
  return wcstoul(str, str_end, base);
}

inline auto CharConstants<char>::toULongLong(const char *str, char **str_end,
                                             int base) -> unsigned long long {
  return strtoull(str, str_end, base);
}
inline auto CharConstants<wchar_t>::toULongLong(const wchar_t *str,
                                                wchar_t **str_end, int base)
    -> unsigned long long {
  return wcstoull(str, str_end, base);
}

inline auto CharConstants<char>::toFloat(const char *str, char **str_end)
    -> float {
  return strtof(str, str_end);
}
inline auto CharConstants<wchar_t>::toFloat(const wchar_t *str,
                                            wchar_t **str_end) -> float {
  return wcstof(str, str_end);
}

inline auto CharConstants<char>::toDouble(const char *str, char **str_end)
    -> double {
  return strtod(str, str_end);
}
inline auto CharConstants<wchar_t>::toDouble(const wchar_t *str,
                                             wchar_t **str_end) -> double {
  return wcstod(str, str_end);
}

inline auto CharConstants<char>::toLongDouble(const char *str, char **str_end)
    -> long double {
  return strtold(str, str_end);
}
inline auto CharConstants<wchar_t>::toLongDouble(const wchar_t *str,
                                                 wchar_t **str_end)
    -> long double {
...
#define ARGUM_DEFINE_MESSAGES(type, pr)                                        \
  template <> struct Messages<type> {                                          \
    static constexpr auto unrecognizedOptionError() {                          \
      return pr##"unrecognized option: {1}";                                   \
    }                                                                          \
    static constexpr auto ambiguousOptionError() {                             \
      return pr##"ambigous option: {1}, candidates: {2}";                      \
    }                                                                          \
    static constexpr auto missingOptionArgumentError() {                       \
      return pr##"argument required for option: {1}";                          \
    }                                                                          \
    static constexpr auto extraOptionArgumentError() {                         \
      return pr##"extraneous argument for option: {1}";                        \
    }                                                                          \
    static constexpr auto extraPositionalError() {                             \
      return pr##"unexpected argument: {1}";                                   \
    }                                                                          \
    static constexpr auto validationError() {                                  \
      return pr##"invalid arguments: {1}";                                     \
    }                                                                          \
    static constexpr auto errorReadingResponseFile() {                         \
      return pr##"error reading response file \"{1}\": {2}";                   \
    }                                                                          \
    static constexpr auto option() { return pr##"option"; }                    \
    static constexpr auto positionalArg() {                                    \
      return pr##"positional argument";                                        \
    }                                                                          \
    static constexpr auto usageStart() { return pr##"Usage: "; }               \
    static constexpr auto defaultArgName() { return pr##"ARG"; }               \
    static constexpr auto positionalHeader() {                                 \
      return pr##"positional arguments:";                                      \
    }                                                                          \
    static constexpr auto optionsHeader() { return pr##"options:"; }           \
    static constexpr auto listJoiner() { return pr##", "; }                    \
    static constexpr auto expectedValueNotPresent() {                          \
      return pr##"expected value is not present";                              \
    }                                                                          \
    static constexpr auto notANumber() {                                       \
      return pr##"value \"{1}\" is not a number";                              \
    }                                                                          \
    static constexpr auto outOfRange() {                                       \
      return pr##"value \"{1}\" is out of range";                              \
    }                                                                          \
    static constexpr auto notAValidChoice() {                                  \
      return pr##"value \"{1}\" is not one of the valid choices {{{2}}";       \
    }                                                                          \
    static constexpr auto itemUnrestricted() {                                 \
      return pr##"{1} {2} can occur any number of times";                      \
    }                                                                          \
    static constexpr auto itemMustBePresent() {                                \
      return pr##"{1} {2} must be present";                                    \
    }                                                                          \
    static constexpr auto itemMustNotBePresent() {                             \
      return pr##"{1} {2} must not be present";                                \
    }                                                                          \
    static constexpr auto itemMustBePresentGE(unsigned val) {                  \
      switch (val) {                                                           \
      case 0:                                                                  \
        return itemUnrestricted();                                             \
      case 1:                                                                  \
        return itemMustBePresent();                                            \
      default:                                                                 \
        return pr##"{1} {2} must occur at least {3} times";                    \
      }                                                                        \
    }                                                                          \
    static constexpr auto itemMustBePresentG(unsigned val) {                   \
      ...
      case 0:                                                                  \
        return itemMustBePresent();                                            \
      default:                                                                 \
        return pr##"{1} {2} must occur more than {3} times";                   \
      }                                                                        \
    }                                                                          \
    static constexpr auto itemMustBePresentLE(unsigned val) {                  \
      ...
      switch (val) {                                                           \
      case 0:                                                                  \
        return itemMustNotBePresent();                                         \
      case 1:                                                                  \
        return pr##"{1} {2} must occur at most once";                          \
      default:                                                                 \
        return pr##"{1} {2} must occur at most {3} times";                     \
      case inf:                                                                \
        return itemUnrestricted();                                             \
      }                                                                        \
    }                                                                          \
    static constexpr auto itemMustBePresentL(unsigned val) {                   \
      ...
      case 1:                                                                  \
        return itemMustNotBePresent();                                         \
      default:                                                                 \
        return pr##"{1} {2} must occur less than {3} times";                   \
      case inf:                                                                \
        return itemUnrestricted();                                             \
      }                                                                        \
    }                                                                          \
    static constexpr auto itemMustBePresentEQ(unsigned val) {                  \
      ...
      case 1:                                                                  \
        return itemMustBePresent();                                            \
      default:                                                                 \
        return pr##"{1} {2} must occur {3} times";                             \
      }                                                                        \
    }                                                                          \
    static constexpr auto itemMustBePresentNEQ(unsigned val) {                 \
      ...
      case 0:                                                                  \
        return itemMustBePresent();                                            \
      default:                                                                 \
        return pr##"{1} {2} must NOT occur {3} times";                         \
      case inf:                                                                \
        return itemUnrestricted();                                             \
      }                                                                        \
    }                                                                          \
...
auto append(std::basic_string<Char> &dest, T &&val) -> std::basic_string<Char> &
  requires(
      std::is_same_v<decltype(dest += std::forward<T>(val)), decltype(dest)>)
{
...
inline auto append(std::string &dest, const std::wstring &str)
    -> std::string & {
  return append(dest, str.c_str());
}

inline auto append(std::string &dest, const std::wstring_view &str)
    -> std::string & {
...
inline auto append(std::wstring &dest, const std::string &str)
    -> std::wstring & {
  return append(dest, str.c_str());
}

inline auto append(std::wstring &dest, const std::string_view &str)
    -> std::wstring & {
  return append(dest, std::string(str));
}

template <class T, class Char>
concept StringAppendable =
    Character<Char> && requires(std::basic_string<Char> &str, T &&val) {
      {
        append(str, std::forward<T>(val))
      } -> std::same_as<std::basic_string<Char> &>;
    };
...
template <Character Char, StringAppendable<Char> First,
          StringAppendable<Char>... Rest>
auto appendByIndex(std::basic_string<Char> &dest, size_t idx, First &&first,
                   Rest &&...rest) {
...
template <Character Char>
auto parseFormatPlaceholder(const Char *first, const Char *last)
    -> std::optional<size_t> {

  [[maybe_unused]] char
      buffer[16 * MB_LEN_MAX]; // 16 characters for placeholder number ought to
                               // be enough :)
...
auto wordWrap(T &&input, unsigned maxLength, unsigned indent = 0,
              unsigned firstLineOffset = 0)
    -> std::basic_string<CharTypeOf<T>> {
...
public:
  ...
  FlatSet(const value_type &value)
      : m_data(1, value) {}
  FlatSet(value_type &&value) { this->m_data.emplace_back(std::move(value)); }
  FlatSet(std::initializer_list<value_type> values)
      : m_data(values) {
    std::sort(this->m_data.begin(), this->m_data.end(), Comparator());
  }

  auto insert(T val) -> std::pair<iterator, bool> {
    auto it = std::lower_bound(this->m_data.begin(), this->m_data.end(), val,
                               Comparator());
  ...
  template <class Arg> auto find(const Arg &key) const -> const_iterator {
    auto it = std::lower_bound(this->m_data.begin(), this->m_data.end(), key,
                               Comparator());
...
template <class Key, class Value> class FlatMap {
public:
  ...
  public:
    ...
    const Key &key() const noexcept { return this->first; }

    const Value &value() const noexcept { return this->second; }

    Value &value() noexcept { return this->second; }
  };
  using iterator = typename std::vector<value_type>::iterator;
  using const_iterator = typename std::vector<value_type>::const_iterator;
...
public:
  ...
  template <class KeyArg> auto operator[](const KeyArg &key) -> Value & {
    auto it = std::lower_bound(this->m_data.begin(), this->m_data.end(), key,
                               Comparator());
    if (it == this->m_data.end() || it->key() != key)
  ...
  template <class KeyArg> auto find(const KeyArg &key) const -> const_iterator {
    auto it = std::lower_bound(this->m_data.begin(), this->m_data.end(), key,
                               Comparator());
    if (it == this->m_data.end() || it->key() != key)
      return this->m_data.end();
  ...
  auto lower_bound(const KeyArg &key) const -> const_iterator {
    return std::lower_bound(this->m_data.begin(), this->m_data.end(), key,
                            Comparator());
  }
...
auto findMatchOrMatchingPrefixRange(const FlatMap<Key, Value> &map, Arg arg)
  ...
  //  2. contains a run of items which start with arg followed by ones which
  //  don't
  //  3. is empty
...
ARGUM_MOD_EXPORTED
template <class Char> class BasicValidationData {
public:
...
template <class T, class Char>
concept ParserValidator =
    std::is_invocable_r_v<bool, T, const BasicValidationData<Char>>;

template <class T, class Char>
concept DescribableParserValidator =
    ParserValidator<T, Char> && requires(const T &val) {
      { describe(val) } -> std::same_as<std::basic_string<Char>>;
    };

template <class Char, class First, class... Rest>
constexpr bool CompatibleParserValidators =
    ParserValidator<First, Char> && (ParserValidator<Rest, Char> && ...);

template <class T>
concept AnyParserValidator =
    ParserValidator<T, char> || ParserValidator<T, wchar_t>;
...
template <AnyParserValidator Impl> class NotValidator {
public:
  template <class X>
  NotValidator(X &&x)
    requires(std::is_convertible_v<X &&, Impl>)
      : m_impl(std::forward<X>(x)) {}
  ...
  auto operator!() const -> const Impl & { return m_impl; }
...
ARGUM_MOD_EXPORTED
template <AnyParserValidator Validator> auto operator!(const Validator &val) {
  return NotValidator<std::remove_cvref_t<Validator>>(val);
}

ARGUM_MOD_EXPORTED
template <AnyParserValidator Validator> auto oppositeOf(Validator &&val) {
  return NotValidator<std::remove_cvref_t<Validator>>(val);
}
...
public:
  CombinedValidator(std::tuple<Args...> &&args)
      : m_items(std::move(args)) {}
  template <class... CArgs>
    requires(std::is_constructible_v<TupleType, CArgs && ...>)
  CombinedValidator(CArgs &&...args)
      : m_items(std::forward<CArgs>(args)...) {}
  ...
  auto operator()(const BasicValidationData<Char> &data) const -> bool {
    if constexpr (Comb == ValidatorCombination::And) {
      return std::apply(
          [&data](const Args &...args) -> bool { return (args(data) && ...); },
          this->m_items);
  ...
  auto items() const -> const TupleType & { return m_items; }

private:
  template <size_t N, class Char>
    requires(CompatibleParserValidators<Char, Args...>)
  auto evalOneOrNone(const BasicValidationData<Char> &data,
  ...
  template <size_t N, class Char>
    requires(CompatibleParserValidators<Char, Args...>)
  auto evalAllOrNone(const BasicValidationData<Char> &data,
                     unsigned &counter) const {
...
template <ValidatorCombination Comb, class V1, class V2>
  requires(AnyCompatibleParserValidators<std::remove_cvref_t<V1>,
                                         std::remove_cvref_t<V2>>)
auto combine(V1 &&v1, V2 &&v2) {
  using R1 = std::remove_cvref_t<V1>;
  using R2 = std::remove_cvref_t<V2>;
  return CombinedValidator<Comb, R1, R2>(std::forward<V1>(v1),
                                         std::forward<V2>(v2));
}
...
auto combine(V1 &&v1, CombinedValidator<Comb, Args2...> &&v2) {
  using R1 = std::remove_cvref_t<V1>;
  return CombinedValidator<Comb, R1, Args2...>(std::tuple_cat(
      std::tuple<R1>(std::forward<V1>(v1)), std::move(v2.items())));
}
...
auto combine(CombinedValidator<Comb, Args1...> &&v1, V2 &&v2) {
  using R2 = std::remove_cvref_t<V2>;
  return CombinedValidator<Comb, Args1..., R2>(std::tuple_cat(
      std::move(v1.items()), std::tuple<R2>(std::forward<V2>(v2))));
}
...
auto operator&&(V1 &&v1, V2 &&v2) {
  return combine<ValidatorCombination::And>(std::forward<V1>(v1),
                                            std::forward<V2>(v2));
}

ARGUM_MOD_EXPORTED
template <class First, class... Rest>
  requires(AnyCompatibleParserValidators<std::remove_cvref_t<First>,
...
template <class V1, class V2>
  requires(AnyCompatibleParserValidators<std::remove_cvref_t<V1>,
                                         std::remove_cvref_t<V2>>)
auto operator||(V1 &&v1, V2 &&v2) {
  return combine<ValidatorCombination::Or>(std::forward<V1>(v1),
                                           std::forward<V2>(v2));
}
...
auto onlyOneOf(First &&first, Rest &&...rest) {
  ...
  return CombinedValidator<ValidatorCombination::OnlyOne,
...
auto oneOrNoneOf(First &&first, Rest &&...rest) {
  return CombinedValidator<ValidatorCombination::OneOrNone,
                           std::remove_cvref_t<First>,
                           std::remove_cvref_t<Rest>...>(
      std::forward<First>(first), std::forward<Rest>(rest)...);
}
...
auto allOrNoneOf(First &&first, Rest &&...rest) {
  return CombinedValidator<ValidatorCombination::AllOrNone,
                           std::remove_cvref_t<First>,
                           std::remove_cvref_t<Rest>...>(
      std::forward<First>(first), std::forward<Rest>(rest)...);
}
...
class ItemOccurs {
public:
  ItemOccurs(std::basic_string_view<Char> name, unsigned count)
      : m_name(name),
        m_count(count) {}
  ...
  auto operator!() const {
    if constexpr (std::is_same_v<Comp, std::greater_equal<unsigned>>)
      return ItemOccurs<Char, IsOption, std::less<unsigned>>(this->m_name,
                                                             this->m_count);
    else if constexpr (std::is_same_v<Comp, std::less_equal<unsigned>>)
      return ItemOccurs<Char, IsOption, std::greater<unsigned>>(this->m_name,
                                                                this->m_count);
    else if constexpr (std::is_same_v<Comp, std::greater<unsigned>>)
      return ItemOccurs<Char, IsOption, std::less_equal<unsigned>>(
          this->m_name, this->m_count);
    else if constexpr (std::is_same_v<Comp, std::less<unsigned>>)
      return ItemOccurs<Char, IsOption, std::greater_equal<unsigned>>(
          this->m_name, this->m_count);
    else if constexpr (std::is_same_v<Comp, std::equal_to<unsigned>>)
      return ItemOccurs<Char, IsOption, std::not_equal_to<unsigned>>(
          this->m_name, this->m_count);
    else if constexpr (std::is_same_v<Comp, std::not_equal_to<unsigned>>)
      return ItemOccurs<Char, IsOption, std::equal_to<unsigned>>(this->m_name,
                                                                 this->m_count);
  }
  ...
  friend auto describe(const ItemOccurs &val) {
    ...
    if constexpr (std::is_same_v<Comp, std::greater_equal<unsigned>>) {
      return format(Messages::itemMustBePresentGE(val.m_count), typeName,
                    val.m_name, val.m_count);
    } else if constexpr (std::is_same_v<Comp, std::less_equal<unsigned>>) {
      return format(Messages::itemMustBePresentLE(val.m_count), typeName,
                    val.m_name, val.m_count);
    } else if constexpr (std::is_same_v<Comp, std::greater<unsigned>>) {
      return format(Messages::itemMustBePresentG(val.m_count), typeName,
                    val.m_name, val.m_count);
    } else if constexpr (std::is_same_v<Comp, std::less<unsigned>>) {
      return format(Messages::itemMustBePresentL(val.m_count), typeName,
                    val.m_name, val.m_count);
    } else if constexpr (std::is_same_v<Comp, std::equal_to<unsigned>>) {
      return format(Messages::itemMustBePresentEQ(val.m_count), typeName,
                    val.m_name, val.m_count);
    } else if constexpr (std::is_same_v<Comp, std::not_equal_to<unsigned>>) {
      return format(Messages::itemMustBePresentNEQ(val.m_count), typeName,
                    val.m_name, val.m_count);
    }
  }
...
ARGUM_MOD_EXPORTED template <StringLike S> auto optionPresent(S name) {
  return ItemOccurs<CharTypeOf<S>, true, std::greater<unsigned>>(name, 0);
}
ARGUM_MOD_EXPORTED template <StringLike S> auto optionAbsent(S name) {
...
auto optionOccursAtLeast(S name, unsigned count) {
  return ItemOccurs<CharTypeOf<S>, true, std::greater_equal<unsigned>>(name,
                                                                       count);
}
ARGUM_MOD_EXPORTED template <StringLike S>
auto optionOccursAtMost(S name, unsigned count) {
  return ItemOccurs<CharTypeOf<S>, true, std::less_equal<unsigned>>(name,
                                                                    count);
}
ARGUM_MOD_EXPORTED template <StringLike S>
...
auto optionDoesntOccurExactly(S name, unsigned count) {
  return ItemOccurs<CharTypeOf<S>, true, std::not_equal_to<unsigned>>(name,
                                                                      count);
}

ARGUM_MOD_EXPORTED template <StringLike S> auto positionalPresent(S name) {
  return ItemOccurs<CharTypeOf<S>, false, std::greater<unsigned>>(name, 0);
}
ARGUM_MOD_EXPORTED template <StringLike S> auto positionalAbsent(S name) {
  return ItemOccurs<CharTypeOf<S>, false, std::equal_to<unsigned>>(name, 0);
}
ARGUM_MOD_EXPORTED template <StringLike S>
auto positionalOccursAtLeast(S name, unsigned count) {
  return ItemOccurs<CharTypeOf<S>, false, std::greater_equal<unsigned>>(name,
                                                                        count);
}
ARGUM_MOD_EXPORTED template <StringLike S>
auto positionalOccursAtMost(S name, unsigned count) {
  return ItemOccurs<CharTypeOf<S>, false, std::less_equal<unsigned>>(name,
                                                                     count);
}
ARGUM_MOD_EXPORTED template <StringLike S>
...
auto positionalDoesntOccurExactly(S name, unsigned count) {
  return ItemOccurs<CharTypeOf<S>, false, std::not_equal_to<unsigned>>(name,
                                                                       count);
}
...
ARGUM_MOD_EXPORTED
template <class Char> class BasicOptionNames final {
...
public:
  template <StringConvertibleOf<CharType> First,
            StringConvertibleOf<CharType>... Rest>
  BasicOptionNames(First &&first, Rest &&...rest)
      : m_values{makeString(std::forward<First>(first)),
                 makeString(std::forward<Rest>(rest))...} {}

  auto main() const -> const StringType & { return this->m_values[0]; }

  auto all() const -> const std::vector<StringType> & { return this->m_values; }
...
enum class OptionArgumentKind { None, Optional, Required };

ARGUM_MOD_EXPORTED class Quantifier {
public:
  static constexpr unsigned infinity = std::numeric_limits<unsigned>::max();

  constexpr Quantifier(unsigned val)
      : m_min(val),
        m_max(val) {}
  constexpr Quantifier(unsigned min, unsigned max)
      : m_min(min),
        m_max(max) {
    if (min > max)
  ...
  constexpr auto min() const { return this->m_min; }

  constexpr auto max() const { return this->m_max; }
...
ARGUM_MOD_EXPORTED inline constexpr Quantifier
    zeroOrMoreTimes(0, Quantifier::infinity);
ARGUM_MOD_EXPORTED inline constexpr Quantifier
    oneOrMoreTimes(1, Quantifier::infinity);
...
ARGUM_MOD_EXPORTED
template <class Char> class BasicParsingException : public std::exception {
private:
  using StringsType =
      std::conditional_t<std::is_same_v<Char, char>, std::tuple<std::string>,
...
public:
  ...
  auto code() const noexcept -> Error { return m_code; }
  ...
  auto as() noexcept -> Derived * {
    return const_cast<Derived *>(
        const_cast<const BasicParsingException *>(this)->as<Derived>());
  }
...
#define ARGUM_IMPLEMENT_EXCEPTION(type, base, code)                            \
  ...
  [[noreturn]] auto raise() const -> void override {                           \
    ARGUM_RAISE_EXCEPTION(*this);                                              \
  }
...
ARGUM_MOD_EXPORTED template <class T>
using FailureType = std::in_place_type_t<T>;

ARGUM_MOD_EXPORTED template <class T> inline constexpr FailureType<T> Failure{};

ARGUM_MOD_EXPORTED
template <class Char, class T> class [[nodiscard]] BasicExpected {
...
public:
  ...
  BasicExpected(ParsingExceptionPtr err)
      : m_impl(BasicExpected::validate(err)) {}
  ...
  auto value() const & -> ConstLValueReference {
    return std::visit(
        [](const auto &val) -> ConstLValueReference {
          if constexpr (std::is_same_v<std::remove_cvref_t<decltype(val)>,
                                       ValueType>) {
            if constexpr (!std::is_same_v<T, void>)
              return val;
  ...
  auto error() const -> ParsingExceptionPtr {
    return std::visit(
        [](const auto &val) -> ParsingExceptionPtr {
          if constexpr (std::is_same_v<std::remove_cvref_t<decltype(val)>,
                                       ParsingExceptionPtr>) {
            return val;
          } else {
...
ARGUM_MOD_EXPORTED template <class T>
using WExpected = BasicExpected<wchar_t, T>;

template <class T, class Char, class Ret, class... Args>
concept IsHandler =
    std::is_invocable_v<std::remove_cvref_t<T>, Args...> &&
    (std::is_same_v<Ret, decltype(std::declval<std::remove_cvref_t<T>>()(
                             std::declval<Args>()...))> ||
     std::is_same_v<BasicExpected<Char, Ret>,
  ...
  requires(IsHandler<decltype(h), Char, Ret, Args...>)
{
  ...
  if constexpr (std::is_invocable_r_v<BasicExpected<Char, Ret>, InHandlerType,
                                      Args...>) {
  ...
  if constexpr (std::is_invocable_r_v<Ret, InHandlerType, Args...>) {
    ...
    InHandlerType handler(std::forward<H>(h));
    return [=](Args... args) -> Ret { return handler(args...).value(); };
  }
#endif
}
...
#define ARGUM_PROPAGATE_ERROR(expr)                                            \
  if (auto err = (expr).error()) {                                             \
    return err;                                                                \
  }
#define ARGUM_CHECK_RESULT_IMPL(temp, var, expr)                               \
  decltype(auto) temp = (expr);                                                \
  ARGUM_PROPAGATE_ERROR(temp);                                                 \
  var = *temp
#define ARGUM_CHECK_RESULT(var, expr)                                          \
  ARGUM_CHECK_RESULT_IMPL(ARGUM_UNIQUE_NAME(argum_check_result), var, expr)
#define ARGUM_THROW(type, ...)                                                 \
  return { Failure<type> __VA_OPT__(, ) __VA_ARGS__ }
...
ARGUM_MOD_EXPORTED
template <class Char> class BasicTokenizer final {
...
private:
  ...
  friend auto operator&(PrefixType lhs, PrefixType rhs) -> PrefixType {
    return PrefixType(std::underlying_type_t<PrefixType>(lhs) &
                      std::underlying_type_t<PrefixType>(rhs));
  }
  friend auto operator&=(PrefixType &lhs, PrefixType rhs) -> PrefixType & {
    lhs = (lhs & rhs);
    return lhs;
...
public:
  ...
  struct OptionToken {
    unsigned
        argIdx;   // index of the argument containing the token in command line
    unsigned idx; // index of the option
    StringType usedName; // specific option name used
    std::optional<StringViewType>
        argument; // argument if included with the otpion via = syntax
  ...
  enum TokenResult { Continue = 0, StopAfter = 0b10, StopBefore = 0b11 };
  ...
  public:
    ...
    auto addLongPrefix(First &&first, Rest &&...rest) & -> Settings & {
      std::initializer_list<StringType> values = {
          makeString(std::forward<First>(first)),
          makeString(std::forward<Rest>(rest))...};
      for (auto &value : values) {
        ...
        if (!inserted) {
          if ((type & ShortPrefix) == ShortPrefix)
            ARGUM_INVALID_ARGUMENT(
                "the same prefix cannot be used for long and short options");
        }
        type |= LongPrefix;
      }
      ++m_lastPrefixId;
    ...
    template <StringConvertibleOf<CharType> First,
              StringConvertibleOf<CharType>... Rest>
    auto addLongPrefix(First &&first, Rest &&...rest) && -> Settings && {
      return std::move(static_cast<Settings *>(this)->addLongPrefix(
          std::forward<First>(first), std::forward<Rest>(rest)...));
    }
    ...
    template <StringConvertibleOf<CharType> First,
              StringConvertibleOf<CharType>... Rest>
    auto addShortPrefix(First &&first, Rest &&...rest) && -> Settings && {
      return std::move(static_cast<Settings *>(this)->addShortPrefix(
          std::forward<First>(first), std::forward<Rest>(rest)...));
    }
    ...
    template <StringConvertibleOf<CharType> First,
              StringConvertibleOf<CharType>... Rest>
    auto addOptionTerminator(First &&first, Rest &&...rest) && -> Settings && {
      return std::move(static_cast<Settings *>(this)->addOptionTerminator(
          std::forward<First>(first), std::forward<Rest>(rest)...));
    }
    ...
    static auto windowsLong() -> Settings {
      Settings ret;
      ret.addLongPrefix(CharConstants::slash, CharConstants::dash,
                        CharConstants::doubleDash)
          .addOptionTerminator(CharConstants::doubleDash)
          .addValueDelimiter(CharConstants::colon);
      return ret;
    }
...
public:
  ...
  auto add(const OptionNames &names) {
    ...
    for (auto &opt : names.all()) {
      ...
      if ((findResult->type & LongPrefix) == LongPrefix) {
        this->add(this->m_longs[findResult->index],
                  opt.substr(findResult->size), currentIndex);
      } else if ((findResult->type & ShortPrefix) == ShortPrefix) {
        if (opt.size() - findResult->size == 1)
          this->add(this->m_singleShorts[findResult->index],
                    opt[findResult->size], currentIndex);
        else
          this->add(this->m_multiShorts[findResult->index],
                    opt.substr(findResult->size), currentIndex);
      } else {
        ARGUM_INVALID_ARGUMENT(
            "option is neither short nor long with currently defined prefixes");
      }
    }
  ...
  auto tokenize(It argFirst, It argLast, Func &&handler) const
      -> ARGUM_EXPECTED(CharType, std::vector<StringType>) {

    static_assert(
        IsHandler<Func, CharType, TokenResult, OptionToken>,
        "handler must handle OptionToken and return correct return type");
    static_assert(
        IsHandler<Func, CharType, TokenResult, ArgumentToken>,
        "handler must handle ArgumentToken and return correct return type");
    static_assert(
        IsHandler<Func, CharType, TokenResult, OptionStopToken>,
        "handler must handle OptionStopToken and return correct return type");
    static_assert(IsHandler<Func, CharType, TokenResult, UnknownOptionToken>,
                  "handler must handle UnknownOptionToken and return correct "
                  "return type");
    static_assert(IsHandler<Func, CharType, TokenResult, AmbiguousOptionToken>,
                  "handler must handle AmbiguousOptionToken and return correct "
                  "return type");
    ...
    unsigned consumed =
        0; // how much of the arg was consumed before invoking a handler
    unsigned unconsumedPrefixSize =
        0; // size of the arg option prefix if it is an option
        ...
        if (auto prefixFindResult = this->findLongestPrefix(arg)) {
          ...
          if (prefixFindResult->size == arg.size()) {
            ...
            if ((type & OptionStop) == OptionStop) {
              noMoreOptions = true;
              ARGUM_CHECK_RESULT(result,
                                 this->callHandler(std::forward<Func>(handler),
                                                   OptionStopToken{argIdx}));
            ...
            if ((type & LongPrefix) == LongPrefix) {
              ARGUM_CHECK_RESULT(
                  result,
                  this->handleLongPrefix(argIdx, arg, prefixFindResult->index,
                                         prefixFindResult->size,
                                         std::forward<Func>(handler)));
              if (result == TokenResult::StopAfter)
                consumed = unsigned(arg.size());
            } else if ((type & ShortPrefix) == ShortPrefix) {
              ARGUM_CHECK_RESULT(
                  result, this->handleShortPrefix(
                              argIdx, arg, prefixFindResult->index,
                              prefixFindResult->size, consumed, handler));
    ...
    if (argFirst != argLast) {
      ...
      if (consumed == arg.size()) {
        ...
        rest.emplace_back(StringType(arg.substr(0, unconsumedPrefixSize)) +
                          StringType(arg.substr(consumed)));
        ++argFirst;
      }
...
private:
  ...
  auto callHandler(Handler &&handler, Token &&token) const
      -> ARGUM_EXPECTED(CharType, TokenResult) {

    return adaptHandler<CharType, Handler, TokenResult,
                        std::remove_cvref_t<Token>>(
        std::forward<Handler>(handler))(std::forward<Token>(token));
  }

  template <class Func>
  auto handleLongPrefix(unsigned argIdx, StringViewType option,
                        PrefixId prefixId, unsigned nameStart,
                        Func &&handler) const
      -> ARGUM_EXPECTED(CharType, TokenResult) {

    auto [name, arg] = this->splitDelimitedArgument(option, nameStart);
    if (name.size() == 0)
      return this->callHandler(std::forward<Func>(handler),
                               ArgumentToken{argIdx, option});
    ...
    if (mapIt == this->m_longs.end()) {
      return this->callHandler(
          std::forward<Func>(handler),
          UnknownOptionToken{argIdx, std::move(usedName), std::move(arg)});
    }
    auto &longsMap = mapIt->value();

    if (this->m_allowAbrreviation) {
      const auto &[first, last] =
          findMatchOrMatchingPrefixRange(longsMap, name);
      if (last - first == 1) {
        return this->callHandler(std::forward<Func>(handler),
                                 OptionToken{argIdx, first->value(),
                                             std::move(usedName),
                                             std::move(arg)});
      } else if (last != first) {
        StringType actualPrefix(option.substr(0, nameStart));
        std::vector<StringType> candidates(last - first);
        std::transform(first, last, candidates.begin(),
                       [&](const auto &p) { return actualPrefix + p.key(); });
        return this->callHandler(
            std::forward<Func>(handler),
            AmbiguousOptionToken{argIdx, std::move(usedName), std::move(arg),
                                 candidates});
      }
    } else {
  ...
  template <class Func>
  auto handleShortPrefix(unsigned argIdx, StringViewType option,
                         PrefixId prefixId, unsigned nameStart,
                         unsigned &consumed, Func &&handler) const
      -> ARGUM_EXPECTED(CharType, TokenResult) {

    if (auto maybeResult = this->handleShortOption(
            argIdx, option, prefixId, nameStart, consumed, handler)) {
    ...
    if (auto maybeToken = this->matchNumber(argIdx, option, nameStart)) {
      ARGUM_CHECK_RESULT(
          result, this->callHandler(std::forward<Func>(handler), *maybeToken));
    } else {
      ARGUM_CHECK_RESULT(
          result,
          this->callHandler(
              std::forward<Func>(handler),
              UnknownOptionToken{argIdx, StringType(option), std::nullopt}));
    }
  ...
  template <class Func>
  auto handleShortOption(unsigned argIdx, StringViewType option,
                         PrefixId prefixId, unsigned nameStart,
                         unsigned &consumed, Func &&handler) const
      -> std::optional<ARGUM_EXPECTED(CharType, TokenResult)> {
    ...
    do {
      ...
      ARGUM_CHECK_RESULT(
          auto result,
          this->callHandler(std::forward<Func>(handler),
                            OptionToken{argIdx, currentIdx, std::move(usedName),
                                        std::move(arg)}));
      if (result != TokenResult::Continue) {
        if (result == TokenResult::StopAfter)
  ...
  template <class Func>
  auto handleMultiShortOption(unsigned argIdx, StringViewType option,
                              PrefixId prefixId, unsigned nameStart,
                              bool mustMatchExact, Func &&handler) const
      -> ARGUM_EXPECTED(CharType, std::optional<TokenResult>) {
    ...
    if (this->m_allowAbrreviation) {
      const auto &[first, last] =
          findMatchOrMatchingPrefixRange(multiShortsMap, name);
      if (last != first) {
        ...
        if (last - first == 1) {
          if (!mustMatchExact || first->key() == name) {
            ...
            return this->callHandler(
                std::forward<Func>(handler),
                AmbiguousOptionToken{argIdx, std::move(usedName),
                                     std::move(arg), candidates});
          }
        } else {
          StringType actualPrefix(option.substr(0, nameStart));
          std::vector<StringType> candidates;
          ...
          std::transform(first, last, std::back_inserter(candidates),
                         [&](const auto &p) { return actualPrefix + p.key(); });
          return this->callHandler(
              std::forward<Func>(handler),
              AmbiguousOptionToken{argIdx, std::move(usedName), std::move(arg),
  ...
  auto splitDelimitedArgument(StringViewType option, unsigned nameStart) const
      -> std::pair<StringViewType, std::optional<StringViewType>> {
  ...
  auto matchNumber(unsigned argIdx, StringViewType option,
                   unsigned /*nameStart*/) const
      -> std::optional<ArgumentToken> {
  ...
  static auto add(FlatMap<Value, unsigned> &map, Arg arg, unsigned idx)
      -> void {
  ...
  auto findLongestPrefix(StringViewType arg) const
      -> std::optional<PrefixFindResult> {
...
//   2. A list of M pairs {A,B} each denoting a range of minimum A and maximum B
//   elements (A >=0 && A <= B)
...
// The partitioning is "greedy" - each range consumes as much as it can from
// left to right
template <class S>
...
public:
  ...
  auto paritionsCount() -> size_t { return this->m_ranges.size() + 1; }

  // The minimum size of sequence that can be partitioned
  auto minimumSequenceSize() const { return this->m_minimumExpected; }
  ...
  auto partition(SizeType n) -> std::optional<std::vector<SizeType>> {
    if (n < this->m_minimumExpected)
      return std::nullopt;
    // this "rebases" the sequence so now all we need is to match ranges of {0,
    // length1}, {0, length2},...
    n -= this->m_minimumExpected;

    // because all ranges have minimum 0 and matching is greedy we can just
    // assign smaller of length and remaining n to each adding its minimum
    std::vector<SizeType> results(this->paritionsCount());
    std::transform(this->m_ranges.begin(), this->m_ranges.end(),
                   results.begin(), [&n](const auto range) {
...
class SimpleFile {
public:
  SimpleFile(const std::filesystem::path &path, const char *mode,
             std::error_code &ec) {
#ifndef _MSC_VER
  ...
  auto eof() const -> bool { return feof(m_fp) != 0; }
...
ARGUM_MOD_EXPORTED
template <class Char> class BasicResponseFileReader {
public:
  using CharType = Char;
  using StringType = std::basic_string<Char>;
  using StringViewType = std::basic_string_view<Char>;
  ...
  struct Exception : public BasicParsingException<CharType> {
    ARGUM_IMPLEMENT_EXCEPTION(Exception, BasicParsingException<CharType>,
                              Error::ResponseFileError)

    Exception(const std::filesystem::path &filename_, std::error_code error_)
        : BasicParsingException<CharType>(
              ErrorCode, format(Messages<CharType>::errorReadingResponseFile(),
                                filename_.native(), error_.message())),
          filename(filename_),
          error(error_) {}
...
public:
  BasicResponseFileReader(char prefix) { m_prefixes.emplace_back(1, prefix); }
  BasicResponseFileReader(StringType prefix)
      : m_prefixes(1, prefix) {
    m_prefixes.emplace_back(std::move(prefix));
  }
  BasicResponseFileReader(std::initializer_list<StringType> prefixes)
      : m_prefixes(prefixes) {}

  auto expand(int argc, CharType **argv)
      -> ARGUM_EXPECTED(CharType, std::vector<StringType>) {
    return expand(makeArgSpan(argc, argv));
  }

  template <class Splitter>
  auto expand(int argc, CharType **argv, Splitter &&splitter)
      -> ARGUM_EXPECTED(CharType, std::vector<StringType>) {
    return expand(makeArgSpan(argc, argv), std::forward<Splitter>(splitter));
  }

  template <ArgRange<CharType> Args>
  auto expand(const Args &args)
      -> ARGUM_EXPECTED(CharType, std::vector<StringType>) {
  ...
  auto expand(const Args &args, Splitter &&splitter)
      -> ARGUM_EXPECTED(CharType, std::vector<StringType>)
    requires(
        std::is_invocable_v<decltype(splitter), StringType &&,
                            std::back_insert_iterator<std::vector<StringType>>>)
  {
    ...
    for (StringViewType arg : args) {
      ...
      while (!stack.empty()) {
        ...
        ARGUM_PROPAGATE_ERROR(this->handleArg(
            *entry.current, ret, stack, std::forward<Splitter>(splitter)));
        ++entry.current;
      }
    }
    return ret;
...
private:
  ...
  auto handleArg(StringViewType arg, std::vector<StringType> &dest,
                 std::stack<StackEntry> &stack, Splitter &&splitter)
      -> ARGUM_EXPECTED(CharType, void) {

    auto foundIt =
        std::find_if(this->m_prefixes.begin(), this->m_prefixes.end(),
                     [arg](const StringViewType &prefix) {
    ...
    if (foundIt != this->m_prefixes.end()) {
      ...
      ARGUM_PROPAGATE_ERROR(this->readResponseFile(
          filename, nextEntry.items, std::forward<Splitter>(splitter)));
  ...
  template <class Splitter>
  static auto readResponseFile(StringViewType filename,
                               std::vector<StringType> &dest,
                               Splitter &&splitter)
      -> ARGUM_EXPECTED(CharType, void) {
...
ARGUM_MOD_EXPORTED
template <Character Char> class BasicColorMaker {
private:
  ...
  template <size_t N> static constexpr auto append(Str<N> prev, Char c) {
    Str<N + 1> ret;
    std::copy(std::begin(prev.wrapped), std::begin(prev.wrapped) + N - 1,
              std::begin(ret.wrapped));
    ret.wrapped[N - 1] = c;
    ret.wrapped[N] = 0;
    return ret;
  }

  template <Color C, size_t N> static constexpr auto encode(Str<N> prev) {
    if constexpr (unsigned(C) < 10) {
      auto ret = mytype::append(
          prev, Char(CharConstants<Char>::digit_0 + unsigned(C)));
      return ret;
    } else {
  ...
  template <Color First, Color... Rest> static constexpr auto doMakeColor() {
    constexpr Str<3> prefix{
        {CharConstants<Char>::esc, CharConstants<Char>::squareBracketOpen, 0}};
...
public:
  template <Color First, Color... Rest> static constexpr auto make() {
    return std::basic_string_view<Char>(
        mytype::storage<First, Rest...>.wrapped);
  }
};
...
ARGUM_MOD_EXPORTED
template <Color First, Color... Rest> constexpr auto makeColor() {
  return BasicColorMaker<char>::template make<First, Rest...>();
}

ARGUM_MOD_EXPORTED
template <Color First, Color... Rest> constexpr auto makeWColor() {
...
ARGUM_MOD_EXPORTED
template <class Char> struct BasicColorScheme {
  using StringViewType = std::basic_string_view<Char>;
...
ARGUM_MOD_EXPORTED
template <Character Char> constexpr BasicColorScheme<Char> nullColorScheme{};
...
ARGUM_MOD_EXPORTED
constexpr auto defaultColorScheme() -> const ColorScheme & {
  return basicDefaultColorScheme<char>;
}

ARGUM_MOD_EXPORTED
constexpr auto defaultWColorScheme() -> const WColorScheme & {
  return basicDefaultColorScheme<wchar_t>;
}

ARGUM_MOD_EXPORTED
template <Character Char> class BasicColorizer {
public:
  using CharType = Char;
  using StringViewType = std::basic_string_view<CharType>;
  using StringType = std::basic_string<CharType>;
...
private:
  static auto colorize(StringViewType str, StringViewType prefix)
      -> StringType {
    if (!prefix.size())
...
ARGUM_MOD_EXPORTED
template <class Char> class BasicHelpFormatter {
public:
  using CharType = Char;
  using StringViewType = std::basic_string_view<CharType>;
  using StringType = std::basic_string<CharType>;
...
public:
  BasicHelpFormatter(const BasicParser<Char> &parser, StringViewType progName,
    ...
    if (m_layout.width <= m_layout.helpLeadingGap + m_layout.helpNameMaxWidth +
                              m_layout.helpDescriptionGap)
      m_layout.width = m_layout.helpLeadingGap + m_layout.helpNameMaxWidth +
                       m_layout.helpDescriptionGap + 1;
  }
  ...
  auto formatHelp(const std::optional<StringType> &subCommand,
    ...
    if (subCommand &&
        this->m_parser.subCommandMark().positionalIdx == size_t(-1))
      ARGUM_INVALID_ARGUMENT("subcommand must be defined to use this function "
                             "with non null subcommand");
    ...
    if (!helpContent.positionalItems.empty()) {
      ret.append(wordWrap(colorizer.heading(Messages::positionalHeader()),
                          m_layout.width, m_layout.helpLeadingGap));
      for (auto &[name, desc] : helpContent.positionalItems) {
        ret.append({endl}).append(
            this->formatItemHelp(name, desc, helpContent.maxNameLen));
      }
      ret.append(2, endl);
    }

    if (!helpContent.optionItems.empty()) {
      ret.append(wordWrap(colorizer.heading(Messages::optionsHeader()),
                          m_layout.width, m_layout.helpLeadingGap));
  ...
  auto formatSyntax(const std::optional<StringType> &subCommand,
    ...
    if (subCommand && subCommandMark.positionalIdx == size_t(-1))
      ARGUM_INVALID_ARGUMENT("subcommand must be added to use this function "
                             "with non null subcommand");
    ...
    StringType ret =
        this->appendSyntax(join(this->optionsBegin(false),
                                this->optionsEnd(false), space, getSyntax),
                           join(this->positionalsBegin(false),
                                this->positionalsEnd(false), space, getSyntax));
    if (subCommand) {
      ret = this->appendSyntax(std::move(ret), *subCommand);
      ret = this->appendSyntax(std::move(ret),
                               join(this->optionsBegin(true),
                                    this->optionsEnd(true), space, getSyntax));
      ret = this->appendSyntax(
          std::move(ret), join(this->positionalsBegin(true),
                               this->positionalsEnd(true), space, getSyntax));
    } else if (subCommandMark.positionalIdx != size_t(-1)) {
      ret = this->appendSyntax(
          std::move(ret),
          getSyntax(
              this->m_parser.positionals()[subCommandMark.positionalIdx]));
    }
  ...
  auto calculateHelpContent(bool forSubCommand,
    ...
    for (size_t i = 0; i < positionalsSize; ++i) {
      ...
      ret.positionalItems.emplace_back(std::move(name),
                                       pos.formatHelpDescription());
    }
    std::for_each(this->m_parser.options().begin(),
                  this->optionsEnd(forSubCommand), [&](auto &opt) {
                    auto name = opt.formatHelpName(this->m_parser, colorizer);
                    auto length = stringWidth(name);
                    if (length > ret.maxNameLen)
                      ret.maxNameLen = length;
                    ret.optionItems.emplace_back(std::move(name),
                                                 opt.formatHelpDescription());
                  });
    return ret;
  }

  auto formatItemHelp(StringViewType name, StringViewType description,
                      unsigned maxNameLen) const -> StringType {
    constexpr auto space = CharConstants::space;
    constexpr auto endl = CharConstants::endl;

    auto descColumnOffset = this->m_layout.helpLeadingGap + maxNameLen +
                            this->m_layout.helpDescriptionGap;

    StringType ret =
        wordWrap(StringType(this->m_layout.helpLeadingGap, space).append(name),
                 this->m_layout.width, this->m_layout.helpLeadingGap);
    auto lastEndlPos = ret.rfind(endl);
    auto lastLineLen = stringWidth(StringViewType(
        ret.c_str() + (lastEndlPos + 1), ret.size() - (lastEndlPos + 1)));
    ...
    ret.append(wordWrap(description, this->m_layout.width, descColumnOffset,
                        descColumnOffset));
  ...
  auto positionalsBegin(bool forSubCommand) const {
    ...
    if (subCommandMark.positionalIdx != size_t(-1))
      return this->m_parser.positionals().begin() +
             subCommandMark.positionalIdx + 1;
...
private:
  ...
  using OptArgHandler =
      std::function<HandlerReturnType(std::optional<StringViewType>)>;
...
public:
  using Handler = std::variant<NoArgHandler, OptArgHandler, ReqArgHandler>;
  ...
  auto handler(H &&h) & -> BasicOption &
    requires(
        IsHandler<decltype(h), CharType, void> ||
        ...
        IsHandler<decltype(h), CharType, void, StringViewType>)
  {
    ...
    if constexpr (std::is_invocable_v<InHandlerType>) {
      this->m_handler.template emplace<NoArgHandler>(
          adaptHandler<CharType, H, void>(std::forward<H>(h)));
    } else if constexpr (std::is_invocable_v<InHandlerType,
                                             std::optional<StringViewType>>)
      this->m_handler.template emplace<OptArgHandler>(
          adaptHandler<CharType, H, void, std::optional<StringViewType>>(
              std::forward<H>(h)));
    else {
      this->m_handler.template emplace<ReqArgHandler>(
          adaptHandler<CharType, H, void, StringViewType>(std::forward<H>(h)));
    }
    return *this;
  }

  template <class H>
  auto handler(H &&h) && -> BasicOption &&requires(
      IsHandler<decltype(h), CharType, void> ||
      IsHandler<decltype(h), CharType, void, std::optional<StringViewType>> ||
      IsHandler<decltype(h), CharType, void, StringViewType>) {
    return std::move(
        static_cast<BasicOption *>(this)->handler(std::forward<H>(h)));
  }
  ...
  auto requireAttachedArgument(bool val) && -> BasicOption && {
    return std::move(
        static_cast<BasicOption *>(this)->requireAttachedArgument(val));
  }
  ...
  auto formatSyntax(const BasicParser<CharType> &parser,
    ...
    StringType nameAndArg = isLong ? colorizer.longOptionInUsage(mainName)
                                   : colorizer.shortOptionInUsage(mainName);
  ...
  auto formatArgSyntax(bool forLongName, bool forUsage,
                       const Colorizer &colorizer = {}) const -> StringType {
    constexpr auto space = CharConstants::space;
    constexpr auto brop = CharConstants::squareBracketOpen;
    constexpr auto brcl = CharConstants::squareBracketClose;
    ...
    std::visit(
        [&](const auto &handler) {
          ...
          constexpr auto argumentKind =
              BasicOption::template argumentKindOf<HandlerType>();
          auto colorArg = forUsage ? colorizer.optionArgInUsage(this->m_argName)
                                   : colorizer.optionArg(this->m_argName);
  ...
  auto formatHelpName(const BasicParser<CharType> &parser,
    ...
    auto handleName = [&](const StringType &name) {
      ...
      StringType ret =
          isLong ? colorizer.longOption(name) : colorizer.shortOption(name);
...
private:
  OptionNames m_names;
  Handler m_handler = []() -> ARGUM_EXPECTED(CharType, void) {
    return ARGUM_VOID_SUCCESS;
  };
...
public:
  ...
  template <class H>
  auto handler(H &&h) && -> BasicPositional &&requires(
      IsHandler<decltype(h), CharType, void, StringViewType>) {
    return std::move(
        static_cast<BasicPositional *>(this)->handler(std::forward<H>(h)));
  }
...
private:
  StringType m_name;
  Handler m_handler = [](StringViewType) -> ARGUM_EXPECTED(CharType, void) {
    return ARGUM_VOID_SUCCESS;
  };
...
public:
  struct UnrecognizedOption : public ParsingException {
    ...
    UnrecognizedOption(StringViewType option_)
        : ParsingException(
              ErrorCode, format(Messages::unrecognizedOptionError(), option_)),
          option(option_) {}
    StringType option;
  };
  ...
  struct AmbiguousOption : public ParsingException {
    ...
    AmbiguousOption(StringViewType option_,
                    std::vector<StringType> possibilities_)
        : ParsingException(
              ErrorCode,
              format(Messages::ambiguousOptionError(), option_,
                     join(possibilities_.begin(), possibilities_.end(),
                          Messages::listJoiner()))),
          option(option_),
  ...
  struct MissingOptionArgument : public ParsingException {
    ...
    MissingOptionArgument(StringViewType option_)
        : ParsingException(
              ErrorCode,
              format(Messages::missingOptionArgumentError(), option_)),
          option(option_) {}
    StringType option;
  };
  ...
  struct ExtraOptionArgument : public ParsingException {
    ARGUM_IMPLEMENT_EXCEPTION(ExtraOptionArgument, ParsingException,
                              Error::ExtraOptionArgument)

    ExtraOptionArgument(StringViewType option_)
        : ParsingException(
              ErrorCode, format(Messages::extraOptionArgumentError(), option_)),
          option(option_) {}
    StringType option;
  };
  ...
  struct ExtraPositional : public ParsingException {
    ARGUM_IMPLEMENT_EXCEPTION(ExtraPositional, ParsingException,
                              Error::ExtraPositional)

    ExtraPositional(StringViewType value_)
        : ParsingException(ErrorCode,
                           format(Messages::extraPositionalError(), value_)),
          value(value_) {}
    StringType value;
  };
  ...
  struct ValidationError : public ParsingException {
    ...
    ValidationError(StringViewType message)
        : ParsingException(ErrorCode,
                           format(Messages::validationError(), message)) {}
    template <DescribableParserValidator<CharType> Validator>
    ValidationError(Validator validator)
        : ParsingException(ErrorCode, format(Messages::validationError(),
                                             describe(validator))) {}
  };
...
public:
  ...
  BasicParser(Settings settings)
      : m_tokenizer(settings) {}
  ...
  auto add(Option option) -> void {
    ...
    if (added.m_occurs.min() > 0)
      addValidator(
          optionOccursAtLeast(added.m_names.main(), added.m_occurs.min()));
    ++m_updateCount;
  }
  ...
  auto addSubCommand(Positional positional) {
    ...
    this->m_subCommandMark = {this->m_positionals.size() - 1,
                              this->m_options.size()};
  }
  ...
  auto parseUntilUnknown(int argc, CharType **argv) const
      -> ARGUM_EXPECTED(CharType, std::vector<StringType>) {
  ...
  template <ArgIterator<CharType> It>
  auto parseUntilUnknown(It argFirst, It argLast) const
      -> ARGUM_EXPECTED(CharType, std::vector<StringType>) {
  ...
  auto formatUsage(StringViewType progName, unsigned width) const
      -> StringType {
    return this->formatUsage(progName, {}, width, {});
  }

  auto formatUsage(StringViewType progName, const Colorizer &colorizer) const
      -> StringType {
    return this->formatUsage(progName, {}, HelpFormatter::defaultLayout.width,
                             colorizer);
  }

  auto formatUsage(StringViewType progName, unsigned width,
                   const Colorizer &colorizer) const -> StringType {
    return this->formatUsage(progName, {}, width, colorizer);
  }

  auto formatUsage(StringViewType progName,
                   std::optional<StringType> subCommand) const -> StringType {
    return this->formatUsage(progName, subCommand,
                             HelpFormatter::defaultLayout.width, {});
  }

  auto formatUsage(StringViewType progName,
                   std::optional<StringType> subCommand, unsigned width) const
      -> StringType {
    return this->formatUsage(progName, subCommand, width, {});
  }

  auto formatUsage(StringViewType progName,
                   std::optional<StringType> subCommand,
                   const Colorizer &colorizer) const -> StringType {
    return this->formatUsage(progName, subCommand,
                             HelpFormatter::defaultLayout.width, colorizer);
  }

  auto formatUsage(StringViewType progName,
                   std::optional<StringType> subCommand, unsigned width,
                   const Colorizer &colorizer) const -> StringType {
    typename HelpFormatter::Layout layout;
    layout.width = width;
    return HelpFormatter(*this, progName, layout)
        .formatUsage(subCommand, colorizer);
  }
  ...
  auto formatHelp(StringViewType progName,
                  std::optional<StringType> subCommand) {
    return this->formatHelp(progName, subCommand,
                            HelpFormatter::defaultLayout.width, {});
  }
  ...
  auto formatHelp(StringViewType progName, std::optional<StringType> subCommand,
                  unsigned width, const Colorizer &colorizer) const
      -> StringType {
    typename HelpFormatter::Layout layout;
    layout.width = width;
...
private:
  ...
  public:
    ...
    template <ArgIterator<CharType> It>
    auto parse(It argFirst, It argLast, bool stopOnUnknown)
        -> ARGUM_EXPECTED(CharType, std::vector<StringType>) {

      ARGUM_CHECK_RESULT(
          auto ret,
          m_owner.m_tokenizer.tokenize(
              argFirst, argLast,
              [&](auto &&token) -> ARGUM_EXPECTED(
                                    CharType, typename Tokenizer::TokenResult) {
                using TokenType = std::remove_cvref_t<decltype(token)>;

                if constexpr (std::is_same_v<TokenType,
                                             typename Tokenizer::OptionToken>) {

                  ARGUM_PROPAGATE_ERROR(
                      resetOption(token.idx, token.usedName, token.argument));
                  return Tokenizer::Continue;

                } else if constexpr (std::is_same_v<
                                         TokenType,
                                         typename Tokenizer::OptionStopToken>) {
                  ...
                  return Tokenizer::Continue;
                                         ...
                                         TokenType,
                                         typename Tokenizer::ArgumentToken>) {

                  ARGUM_CHECK_RESULT(auto result,
                                     handlePositional(token.value,
                                                      argFirst + token.argIdx,
                                                      argLast));
                  if (!result) {
                    if (stopOnUnknown)
                ...
                } else if constexpr (std::is_same_v<TokenType,
                                                    typename Tokenizer::
                                                        UnknownOptionToken>) {
                  ...
                  ARGUM_PROPAGATE_ERROR(completeOption());
                  ARGUM_THROW(AmbiguousOption, token.name,
                              std::move(token.possibilities));
                }
              }));
      ARGUM_PROPAGATE_ERROR(completeOption());
  ...
  private:
    auto resetOption(unsigned index, StringViewType name,
                     const std::optional<StringViewType> &argument)
        -> ARGUM_EXPECTED(CharType, void) {
      ARGUM_PROPAGATE_ERROR(completeOption());
    ...
    auto completeOption() -> ARGUM_EXPECTED(CharType, void) {
      ...
      ARGUM_PROPAGATE_ERROR(std::visit(
          [&](const auto &handler) -> ARGUM_EXPECTED(CharType, void) {
            using HandlerType = std::remove_cvref_t<decltype(handler)>;
            constexpr auto argumentKind =
                Option::template argumentKindOf<HandlerType>();
    ...
    auto completeOptionUsingArgument(StringViewType argument)
        -> ARGUM_EXPECTED(CharType, bool) {
      ...
      ARGUM_CHECK_RESULT(
          auto ret,
          std::visit(
              [&](const auto &handler) -> ARGUM_EXPECTED(CharType, bool) {
                using HandlerType = std::remove_cvref_t<decltype(handler)>;
                constexpr auto argumentKind =
                    Option::template argumentKindOf<HandlerType>();
                if constexpr (argumentKind == OptionArgumentKind::None) {
                  ...
                  ARGUM_PROPAGATE_ERROR(handler());
                  return false;
                } else if constexpr (argumentKind ==
                                     OptionArgumentKind::Optional) {
    ...
    template <ArgIterator<CharType> It>
    auto handlePositional(StringViewType value, It remainingArgFirst,
                          It argLast) -> ARGUM_EXPECTED(CharType, bool) {
      ...
      if (m_positionalIndex >= 0) {
        ...
        if (m_positionalSizes[unsigned(m_positionalIndex)] >
            m_validationData.positionalCount(current.m_name))
          positional = &current;
      }

      if (!positional) {
        auto next =
            std::find_if(m_positionalSizes.begin() + (m_positionalIndex + 1),
                         m_positionalSizes.end(),
                         [](const unsigned val) { return val > 0; });
    ...
    auto calculateRemainingPositionals(It remainingArgFirst, It argLast) {
      ...
      unsigned remainingPositionalCount =
          countRemainingPositionals(remainingArgFirst, argLast);
      ...
      if (m_positionalIndex >= 0) {
        ...
        if (positional.m_occurs.max() > count) {
          ...
          partitioner.addRange(positional.m_occurs.min(),
                               positional.m_occurs.max());
          --fillStartIndex;
        }
      }
      std::for_each(m_owner.m_positionals.begin() + (m_positionalIndex + 1),
                    m_owner.m_positionals.end(),
      ...
      unsigned maxRemainingPositionalCount =
          std::max(remainingPositionalCount, partitioner.minimumSequenceSize());
      ...
      std::copy(res->begin(), res->end() - 1,
                m_positionalSizes.begin() + fillStartIndex);

      this->m_updateCountAtLastRecalc = m_owner.m_updateCount;
    }

    template <ArgIterator<CharType> It>
    auto countRemainingPositionals(It remainingArgFirst, It argLast)
        -> unsigned {

      unsigned remainingPositionalCount = 0;
      bool currentOptionExpectsArgument = false;
      (void)m_owner.m_tokenizer.tokenize(
          remainingArgFirst, argLast, [&](const auto &token) noexcept {
            using TokenType = std::remove_cvref_t<decltype(token)>;
            ...
            if constexpr (std::is_same_v<TokenType,
              ...
              if (!currentOptionExpectsArgument)
                ...
                currentOptionExpectsArgument = false;
                                     ...
                                     TokenType,
                                     typename Tokenizer::UnknownOptionToken>) {
    ...
    auto validate() -> ARGUM_EXPECTED(CharType, void) {
      ...
      // We could use normal validators for this but it is faster to do it
      // manually
      for (auto idx =
               (m_positionalIndex >= 0 ? unsigned(m_positionalIndex) : 0u);
           idx != unsigned(m_owner.m_positionals.size()); ++idx) {

        auto &positional = m_owner.m_positionals[unsigned(idx)];
        auto validator = positionalOccursAtLeast(positional.m_name,
                                                 positional.m_occurs.min());
        if (!validator(m_validationData))
          ARGUM_THROW(ValidationError, validator);
      }
  ...
  if constexpr (std::is_signed_v<T>) {
    ...
    if constexpr (sizeof(T) <= sizeof(long)) {
      ...
      if constexpr (sizeof(T) < sizeof(res)) {
        if (res < (decltype(res))std::numeric_limits<T>::min() ||
            res > (decltype(res))std::numeric_limits<T>::max())
...
ARGUM_MOD_EXPORTED
template <Character Char> class BasicChoiceParser {
public:
  using StringViewType = std::basic_string_view<Char>;
...
public:
  BasicChoiceParser(Settings settings = Settings())
      : m_options(std::regex_constants::ECMAScript) {
  ...
  auto parse(StringViewType value) const -> ARGUM_EXPECTED(Char, size_t) {
    ...
    auto it =
        std::find_if(this->m_choices.begin(), this->m_choices.end(),
                     [&](const RegexType &regex) {
                       return regex_match(value.begin(), value.end(), regex);
                     });
    if (it == this->m_choices.end()) {
      if (!m_allowElse)
        ARGUM_THROW(ValidationError, format(Messages::notAValidChoice(), value,
                                            this->m_description));
  ...
  auto description() const -> StringType {
    return CharConstants::braceOpen + this->m_description +
           CharConstants::braceClose;
  }

private:
  auto combine(StringType &, StringType &) { return; }

  template <StringLikeOf<Char> First, StringLikeOf<Char>... Rest>
  auto combine(StringType &exp, StringType &desc, First &&first,
               Rest &&...rest) {
  ...
  static auto escape(Arg &&arg) -> StringType {
    ...
    StringType ret;
    regex_replace(
        std::back_inserter(ret), view.begin(), view.end(),
...
private:
  ...
  static inline const RegexType s_needToBeEscaped{
      CharConstants::mustEscapeInRegex};
  static inline const StringType s_escaped =
      CharConstants::regexEscapeReplacement;
};

ARGUM_DECLARE_FRIENDLY_NAMES(ChoiceParser)

ARGUM_MOD_EXPORTED
template <Character Char> class BasicBooleanParser {
public:
  BasicBooleanParser() {
  ...
  auto parse(std::basic_string_view<Char> value) const
      -> ARGUM_EXPECTED(Char, bool) {
...
ARGUM_MOD_EXPORTED
enum class ColorStatus { unknown, forbidden, allowed, required };
...
inline auto environmentColorStatus() -> ColorStatus {
  ...
  if (auto val = getenv("TERM")) {
    ...
    for (auto &start :
         {"screen"sv, "xterm"sv, "vt100"sv, "vt220"sv, "rxvt"sv, "gnome"sv,
          "konsole"sv, "kterm"sv, "alacritty"sv, "console"sv}) {
    ...
    for (auto &end : {"-256"sv}) {
      if (term.size() >= end.size() &&
          term.substr(term.size() - end.size()) == end)
        return ColorStatus::allowed;
    }
  }
...
ARGUM_MOD_EXPORTED
inline auto colorizerForFile(ColorStatus envColorStatus, FILE *fp)
    -> Colorizer {
...
ARGUM_MOD_EXPORTED
inline auto wideColorizerForFile(ColorStatus envColorStatus, FILE *fp)
    -> WColorizer {
  if (shouldUseColor(envColorStatus, fp))
...
inline unsigned terminalWidth(FILE *fp) {
  ...
  int desc = fileno(fp);
  if (isatty(desc) && ioctl(desc, TIOCGWINSZ, &ws) == 0 && ws.ws_col > 0) {
