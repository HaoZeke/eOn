...
struct Vector : public std::array<double, 3> {
  ...
  Vector normalize() const { return *this * (1.0 / this->norm()); }
...
public:
  ...
  BoundingBox(Matrix matrix, const bool periodic[3])
    ...
    if (n_periodic == 0) {
      ...
      // Assign back to the matrix picking the "non-periodic" indices without
      // ifs
    ...
    distances_between_faces_ = Vector{
        periodic_[0] ? std::abs(na.dot(a))
                     : max_positions_[0] - min_positions_[0],
        periodic_[1] ? std::abs(nb.dot(b))
                     : max_positions_[1] - min_positions_[1],
        periodic_[2] ? std::abs(nc.dot(c))
                     : max_positions_[2] - min_positions_[2],
    };
  }

  const Matrix &matrix() const { return this->matrix_; }

  bool periodic(size_t spatial) const { return this->periodic_[spatial]; }
  ...
  Vector cartesian_to_fractional(Vector cartesian) const {
    ...
    if (!periodic_[0]) {
      fractional[0] =
          (cartesian[0] - min_positions_[0]) / distances_between_faces_[0];
    }

    if (!periodic_[1]) {
      fractional[1] =
          (cartesian[1] - min_positions_[1]) / distances_between_faces_[1];
    }

    if (!periodic_[2]) {
      fractional[2] =
          (cartesian[2] - min_positions_[2]) / distances_between_faces_[2];
    }
  ...
  Vector distances_between_faces() const { return distances_between_faces_; }
  ...
  void make_bounding_for(const Vector *points, size_t n_points) {
    ...
    for (int dim = 0; dim < 3; dim++) {
      ...
      if (!periodic_[dim]) {
        ...
        // bounding box
        distances_between_faces_[dim] =
            (max_positions_[dim] - min_positions_[dim]) * 1.01;
...
void cell_list_neighbors(const Vector *points, size_t n_points, BoundingBox box,
                         VesinOptions options, VesinNeighborList &neighbors,
                         size_t &capacity);
...
void brute_force_neighbors(const Vector *points, size_t n_points,
                           BoundingBox box, VesinOptions options,
                           VesinNeighborList &neighbors, size_t &capacity,
                           VesinPairVisitor visitor = nullptr,
                           void *visitor_data = nullptr,
                           double visit_cutoff = 0.0);

void neighbors(const Vector *points, size_t n_points, BoundingBox box,
               VesinOptions options, VesinNeighborList &neighbors);
...
public:
  ...
  void foreach_pair(ThreadPool &thread_pool, size_t n_threads,
                    Function callback);
...
public:
  ...
  size_t length() const { return neighbors.length; }

  void increment_length() { neighbors.length += 1; }
  ...
  void run(size_t active_threads, size_t n_tasks, Function task) {
    ...
    if (workers_.empty() && n_threads_ > 1) {
      for (size_t thread_id = 1; thread_id < n_threads_; thread_id++) {
        workers_.emplace_back([this, thread_id]() { this->worker(thread_id); });
      }
    }
...
struct VerletList {
  ...
  bool needs_rebuild(const Vector *points, size_t n_points,
  ...
  void rebuild(const Vector *points, size_t n_points, const BoundingBox &box);

  /// Filter cached candidates at the exact user cutoff for the current
  /// `points`.
  void filter(const Vector *points, const BoundingBox &box,
              VesinOptions options, VesinNeighborList &neighbors,
              size_t &output_capacity) const;

  /// Number of pairs currently stored in the cached candidate list.
  size_t candidate_count() const { return candidates_.length; }
...
ExtraDataCpu::~ExtraDataCpu() { delete this->verlet_state; }
...
void vesin::cpu::brute_force_neighbors(
    const Vector *points, size_t n_points, BoundingBox box,
    VesinOptions options, VesinNeighborList &raw_neighbors, size_t &capacity,
    VesinPairVisitor visitor, void *visitor_data, double visit_cutoff) {
  ...
  auto neighbors =
      GrowableNeighborList{raw_neighbors, initial_capacity, options};
  neighbors.reset();
  ...
  auto emit = [&](size_t first, size_t second, CellShift shift, Vector vector,
                  double distance2) {
  ...
  auto diagonal = matrix[0][1] == 0 && matrix[0][2] == 0 && matrix[1][0] == 0 &&
      ...
      for (size_t j = i + 1; j < n_points; j++) {
        ...
        if (distance2 < cutoff2) {
          if (visitor != nullptr && distance2 <= visit_cutoff2) {
            visitor(visitor_data, i, j, vector[0], vector[1], vector[2],
                    distance2);
          }
      ...
      for (size_t j = i + 1; j < n_points; j++) {
        ...
        for (size_t k = 0; k < 3; k++) {
          ...
          auto s = -static_cast<double>(
              static_cast<int64_t>(t + std::copysign(0.5, t)));
...
void vesin::cpu::cell_list_neighbors(const Vector *points, size_t n_points,
                                     BoundingBox box, VesinOptions options,
  ...
  auto thread_count =
      std::max<size_t>(1, static_cast<size_t>(options.n_threads));
  ...
  cell_list.foreach_pair(
      thread_pool, thread_count,
      [&](size_t first, size_t second, CellShift shift, size_t thread_id) {
        if (!options.full) {
  ...
  thread_pool.run(
      thread_count, thread_locals.raw.size(),
      [&](size_t local_i, size_t /*thread_id*/) {
        ...
        for (size_t i = 0; i < local_neighbors.length; i++) {
          ...
          if (options.return_vectors) {
            neighbors.neighbors.vectors[index][0] =
                local_neighbors.vectors[i][0];
            neighbors.neighbors.vectors[index][1] =
                local_neighbors.vectors[i][1];
            neighbors.neighbors.vectors[index][2] =
                local_neighbors.vectors[i][2];
          }
        }
      });
...
void vesin::cpu::neighbors(const Vector *points, size_t n_points,
                           BoundingBox box, VesinOptions options,
                           VesinNeighborList &raw_neighbors) {
  validate_algorithm(options);
  ...
  if (options.skin > 0.0) {
    ...
    if (options.algorithm == VesinBruteForce) {
      brute_force_neighbors(points, n_points, std::move(box), options,
                            raw_neighbors, extra.capacity);
    } else {
      cell_list_neighbors(points, n_points, std::move(box), options,
                          raw_neighbors, extra.capacity);
    }
  }
}
...
divmod(std::array<int32_t, 3> a, std::array<size_t, 3> b) {
  ...
  return std::make_tuple(std::array<int32_t, 3>{qx, qy, qz},
...
CellList::CellList(BoundingBox box, double cutoff)
  ...
  auto n_cells = Vector{
      std::clamp(std::trunc(distances_between_faces[0] / cutoff), 1.0,
                 HUGE_VAL),
      std::clamp(std::trunc(distances_between_faces[1] / cutoff), 1.0,
                 HUGE_VAL),
      std::clamp(std::trunc(distances_between_faces[2] / cutoff), 1.0,
                 HUGE_VAL),
  };

  assert(std::isfinite(n_cells[0]) && std::isfinite(n_cells[1]) &&
         std::isfinite(n_cells[2]));
  ...
  if (n_cells_total > MAX_NUMBER_OF_CELLS) {
    ...
    n_cells[2] = std::trunc(
        std::cbrt(MAX_NUMBER_OF_CELLS / (ratio_x_y * ratio_y_z * ratio_y_z)));
  ...
  this->n_search_ = std::array<int32_t, 3>{
      static_cast<int32_t>(
          std::ceil(cutoff * n_cells[0] / distances_between_faces[0])),
      static_cast<int32_t>(
          std::ceil(cutoff * n_cells[1] / distances_between_faces[1])),
      static_cast<int32_t>(
          std::ceil(cutoff * n_cells[2] / distances_between_faces[2])),
  };
...
void CellList::add_point(size_t index, Vector position) {
  ...
  auto cell_index = std::array<int32_t, 3>{
      static_cast<int32_t>(
          std::floor(fractional[0] * static_cast<double>(cells_shape_[0]))),
      static_cast<int32_t>(
          std::floor(fractional[1] * static_cast<double>(cells_shape_[1]))),
      static_cast<int32_t>(
          std::floor(fractional[2] * static_cast<double>(cells_shape_[2]))),
  };
...
template <typename Function>
void CellList::foreach_pair(ThreadPool &thread_pool, size_t n_threads,
                            Function callback) {
  auto total_cells = cells_shape_[0] * cells_shape_[1] * cells_shape_[2];

  thread_pool.run(
      n_threads, total_cells, [&](size_t linear_cell_i, size_t thread_id) {
        ...
        auto cell_i_y =
            static_cast<int32_t>((linear_cell_i / stride_x) % cells_shape_[1]);
        auto cell_i_z = static_cast<int32_t>(linear_cell_i / stride_xy);

        const auto &current_cell =
            this->get_cell({cell_i_x, cell_i_y, cell_i_z});

        // look through each neighboring cell
        for (int32_t delta_x = -n_search_[0]; delta_x <= n_search_[0];
             delta_x++) {
          for (int32_t delta_y = -n_search_[1]; delta_y <= n_search_[1];
               delta_y++) {
            for (int32_t delta_z = -n_search_[2]; delta_z <= n_search_[2];
                 delta_z++) {
              ...
              // only wrap (i.e. call divmod) cell indices in periodic
              // dimensions, skip out-of-bounds cells in non-periodic ones
              ...
              for (int d = 0; d < 3; d++) {
                if (box_.periodic(d)) {
                  ...
                  cell_shift[d] = q;
                  neighbor_cell_i[d] = r;
                } else if (neighbor_cell_i[d] < 0 ||
                           neighbor_cell_i[d] >=
                               static_cast<int32_t>(cells_shape_[d])) {
                  cell_is_valid = false;
                  break;
                }
              }
              ...
              for (const auto &atom_i : current_cell) {
                for (const auto &atom_j : this->get_cell(neighbor_cell_i)) {
                  auto shift =
                      CellShift(cell_shift) + atom_i.shift - atom_j.shift;
                  auto shift_is_zero =
...
CellList::Cell &CellList::get_cell(std::array<int32_t, 3> index) {
  size_t linear_index = (cells_shape_[0] * cells_shape_[1] * index[2]) +
                        (cells_shape_[0] * index[1]) + index[0];
...
template <typename scalar_t, size_t N> using array_ptr = scalar_t (*)[N];

template <typename scalar_t, size_t N>
static array_ptr<scalar_t, N> alloc(array_ptr<scalar_t, N> ptr, size_t size,
                                    size_t new_size) {
  auto *new_ptr = reinterpret_cast<scalar_t(*)[N]>(
      std::realloc(ptr, new_size * sizeof(scalar_t[N])));
  ...
  std::memset(new_ptr + size, 0b11111111,
              (new_size - size) * sizeof(scalar_t[N]));
#endif
...
static scalar_t *alloc(scalar_t *ptr, size_t size, size_t new_size) {
  auto *new_ptr = reinterpret_cast<scalar_t *>(
      std::realloc(ptr, new_size * sizeof(scalar_t)));
...
void GrowableNeighborList::grow(size_t new_size) {
  ...
  auto *new_pairs =
      alloc<size_t, 2>(neighbors.pairs, neighbors.length, new_size);

  int32_t (*new_shifts)[3] = nullptr;
  if (options.return_shifts) {
    new_shifts =
        alloc<int32_t, 3>(neighbors.shifts, neighbors.length, new_size);
  }

  double *new_distances = nullptr;
  if (options.return_distances) {
    new_distances =
        alloc<double>(neighbors.distances, neighbors.length, new_size);
  }

  double (*new_vectors)[3] = nullptr;
  if (options.return_vectors) {
    new_vectors =
        alloc<double, 3>(neighbors.vectors, neighbors.length, new_size);
  }
  ...
  if ((new_pairs == nullptr) ||
    ...
    throw std::runtime_error(
        "could not allocate memory for growing neighbor list");
  }
...
void GrowableNeighborList::sort() {
  ...
  if ((sorted_pairs == nullptr) ||
    ...
    throw std::runtime_error(
        "could not allocate memory for sorting neighbor list");
  }
...
static double corner_point_displacements(const Matrix &box,
                                         const Matrix &ref_box) {
  double delta1 = 0.0;
  double delta2 = 0.0;
  // The eight corners of the box are generated by the binary representation of
  // the index `i` (0-7). When a bit is 1, the corresponding box vector is
  // included in the corner position.
...
cpu::VerletList::~VerletList() { this->clear_candidates(); }
...
void cpu::VerletList::set_options(VesinOptions options) {
  if (options_.cutoff != options.cutoff || options_.skin != options.skin ||
      options_.full != options.full) {
...
bool cpu::VerletList::needs_rebuild(const Vector *points, size_t n_points,
                                    const BoundingBox &box) const {
  if (!has_cache_) {
    return true;
  }
  ...
  auto half_displacement =
      corner_point_displacements(box.matrix(), ref_matrix_) / 2.0;
  ...
  auto half_threshold_sq = (sqrt(half_skin_sq_) - half_displacement) *
                           (sqrt(half_skin_sq_) - half_displacement);
  for (size_t i = 0; i < n_points; i++) {
...
void cpu::VerletList::rebuild(const Vector *points, size_t n_points,
  ...
  auto periodic =
      std::array<bool, 3>{box.periodic(0), box.periodic(1), box.periodic(2)};
  auto candidate_box = BoundingBox(box.matrix(), periodic.data());
  candidate_box.make_bounding_for(reinterpret_cast<const double (*)[3]>(points),
                                  n_points);

  size_t candidate_capacity = 0;
  cpu::cell_list_neighbors(points, n_points, std::move(candidate_box),
                           build_options, candidates_, candidate_capacity);
...
void cpu::VerletList::filter(const Vector *points, const BoundingBox &box,
                             VesinOptions options, VesinNeighborList &neighbors,
                             size_t &output_capacity) const {
  ...
  auto growable =
      cpu::GrowableNeighborList{neighbors, initial_capacity, options};
  growable.reset();

  // The cached list is an over-complete Verlet candidate list. Each call
  // filters candidates with the exact cutoff and requested shift/vector
  // outputs.
...
public:
  ...
  void run(const double (*points)[3], size_t n_points, const double box[3][3],
           const bool periodic[3], VesinOptions options,
           VesinNeighborList &neighbors);
...
private:
  ...
  void rebuild_cache(const double (*points)[3], size_t n_points,
                     const double box[3][3], const bool periodic[3],
                     int32_t device_id, VesinOptions options);
  void filter_neighbors(const double (*d_points)[3], size_t n_points,
                        const double d_box[3][3], VesinOptions options,
                        VesinNeighborList &neighbors) const;
};
...
struct CellListBuffers {
  ...
  int32_t *d_cell_indices =
      nullptr; // [points_capacity] linear cell index per particle
  int32_t *d_particle_shifts =
      nullptr; // [points_capacity * 3] shift applied to wrap into cell

  // Per-cell arrays (device)
  int32_t *d_cell_counts =
      nullptr; // [cells_capacity] number of particles in each cell
  int32_t *d_cell_starts =
      nullptr; // [cells_capacity] starting index in sorted arrays
  int32_t *d_cell_offsets =
      nullptr; // [cells_capacity] working copy for scatter
  ...
  int32_t *d_sorted_indices =
      nullptr; // [points_capacity] original particle indices
  int32_t *d_sorted_shifts =
      nullptr; // [points_capacity * 3] shifts for sorted particles
  int32_t *d_sorted_cell_indices =
      nullptr; // [points_capacity] cell indices in sorted order
...
struct SortBuffers {
  ...
  size_t (*d_pairs_tmp)[2] =
      nullptr; // [capacity] temporary pair indices for sorting
  int32_t (*d_shifts_tmp)[3] =
      nullptr; // [capacity] temporary shifts for sorting
  double *d_distances_tmp =
      nullptr; // [capacity] temporary distances for sorting
  double (*d_vectors_tmp)[3] =
      nullptr; // [capacity * 3] temporary vectors for sorting

  void allocate(size_t n, bool return_shifts, bool return_distances,
                bool return_vectors);
...
struct CudaNeighborListExtras {
  size_t pairs_capacity =
      0; // Capacity for the pair buffers in the VesinNeighborList
  size_t *d_length_ptr = nullptr;      // GPU-side counter
  int32_t *d_cell_check_ptr = nullptr; // GPU-side status code for checking cell
  int32_t *d_overflow_flag =
      nullptr; // GPU-side flag to detect overflow of pair buffers
  ...
  double (*d_inv_box)[3] =
      nullptr; // [3][3] inverse box matrix for general boxes
...
/// @param periodic Array of three booleans indicating periodicity in each
/// dimension.
...
/// needed).
void neighbors(const double (*points)[3], size_t n_points,
               const double box[3][3], const bool periodic[3],
               VesinOptions options, VesinNeighborList &neighbors);

/// Get the `CudaNeighborListExtras` stored inside `VesinNeighborList`'s opaque
/// pointer
...
void allocate_output_buffers(VesinNeighborList &neighbors, size_t n_pairs,
                             VesinOptions options);
...
void vesin::cuda::free_neighbors(VesinNeighborList &neighbors) {
  throw std::runtime_error(
      "CUDA neighbor list generation is not included in this build of vesin");
}

void vesin::cuda::neighbors(const double (*points)[3], size_t n_points,
                            const double box[3][3], const bool periodic[3],
                            VesinOptions options,
                            VesinNeighborList &neighbors) {
  throw std::runtime_error(
...
static size_t resolve_n_threads(int32_t n_threads) {
  ...
  if (omp_num_threads != nullptr) {
    ...
    if (errno == 0 && end != omp_num_threads && *end == '\0' && parsed > 0 &&
        parsed <= INT32_MAX) {
...
extern "C" int vesin_neighbors(const double (*points)[3], size_t n_points,
                               const double box[3][3], const bool periodic[3],
  ...
  if (neighbors->device.type != VesinUnknownDevice &&
      neighbors->device.type != device.type) {
  ...
  try {
    options.n_threads =
        static_cast<int32_t>(resolve_n_threads(options.n_threads));
    ...
    if (device.type == VesinCPU) {
      ...
      vesin::cpu::neighbors(reinterpret_cast<const vesin::Vector *>(points),
                            n_points, std::move(box), options, *neighbors);
    } else if (device.type == VesinCUDA) {
      vesin::cuda::neighbors(points, n_points, box, periodic, options,
                             *neighbors);
    } else {
...
extern "C" int vesin_neighbors_visit(const double (*points)[3], size_t n_points,
                                     const double box[3][3],
                                     const bool periodic[3], VesinDevice device,
                                     VesinOptions options, double visit_cutoff,
  ...
  if (points == nullptr || box == nullptr || neighbors == nullptr ||
      visitor == nullptr) {
    *error_message =
        "`points`, `box`, `neighbors` and `visitor` can not be NULL pointers";
    return EXIT_FAILURE;
  }
  ...
  if (!std::isfinite(visit_cutoff) || visit_cutoff <= 0 ||
      visit_cutoff > options.cutoff) {
    *error_message =
        "visit_cutoff must be positive and not larger than options.cutoff";
    return EXIT_FAILURE;
  }
  ...
  try {
    ...
    vesin::cpu::brute_force_neighbors(
        reinterpret_cast<const vesin::Vector *>(points), n_points,
        std::move(bounding), options, *neighbors, extra.capacity, visitor,
        user_data, visit_cutoff);
  } catch (const std::bad_alloc &) {
    LAST_ERROR = "failed to allocate memory";
    *error_message = LAST_ERROR.c_str();
    return EXIT_FAILURE;
  ...
  try {
    if (neighbors->device.type == VesinUnknownDevice) {
      ...
      throw std::runtime_error("unknown device " +
                               std::to_string(neighbors->device.type) +
                               " when freeing memory");
    }
  } catch (const std::exception &e) {
    ...
    std::cerr << "fatal error in vesin_free, unknown type thrown as exception"
              << std::endl;
    return;
  }
