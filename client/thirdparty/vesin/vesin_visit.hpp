...
inline void brute_force_visit(const double *points, std::size_t n,
  ...
  for (std::size_t i = 0; i < n; i++) {
    ...
    for (std::size_t j = i + 1; j < n; j++) {
      ...
      if (r2 < list_cutoff2) {
        ...
        if (r2 <= visit_cutoff2) {
          visit(static_cast<int32_t>(i), static_cast<int32_t>(j), dx, dy, dz,
                r2);
        }
      }
    }
...
template <typename Visitor>
inline void brute_force_visit_only(const double *points, std::size_t n,
                                   const double w[3], const double inv[3],
                                   double visit_cutoff2, Visitor &&visit) {
  const double w0 = w[0], w1 = w[1], w2 = w[2];
  const double i0 = inv[0], i1 = inv[1], i2 = inv[2];
  for (std::size_t i = 0; i < n; i++) {
    const double xi = points[3 * i];
    ...
    for (std::size_t j = i + 1; j < n; j++) {
      ...
      const double r2 = dx * dx + dy * dy + dz * dz;
      if (r2 <= visit_cutoff2) {
        visit(static_cast<int32_t>(i), static_cast<int32_t>(j), dx, dy, dz, r2);
      }
    }
  }
}
