...
std::vector<std::string> ParallelReplicaJob::run(void) {
  ...
  reactant->con2matter(helper_functions::getRelevantFile(params->main_options.conFilename));
  ...
  *trajectory = *reactant;
  ...
  int stateCheckInterval =
      int(floor(params->parallel_replica_options.state_check_interval / params->dynamics_options.time_step + 0.5));
  int recordInterval =
      int(floor(params->parallel_replica_options.record_interval / params->dynamics_options.time_step + 0.5));
  ...
  for (int step = 1; step <= params->dynamics_options.steps; step++) {
    ...
    if (step % recordInterval == 0 && params->parallel_replica_options.refine_transition) {
    ...
    // simulation
    if (step % stateCheckInterval == 0 || step == params->dynamics_options.steps) {
      ...
      } else if (step + 1 == params->dynamics_options.steps && transitionTime == 0) {
  ...
  fprintf(fileResults, "%lf speedup\n",
          simulationTime / (params->dynamics_options.steps * params->dynamics_options.time_step));
