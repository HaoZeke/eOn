// Isolated TU: rgpot only (no eOn Potential.h) — avoids Cap'n Proto Potential clash.
...
RGPotEngine::RGPotEngine(const RGPotEngineOptions &opt)
  ...
  if (backend_ == "nwchem" || backend_ == "nwchemc" || backend_ == "nwchempot") {
    backend_ = "nwchemc";
    ...
    if (!impl_->nwchem->available())
      throw std::runtime_error(
          ...
          "RGPOT_NWCHEMC_ENGINE or [RGPot] engine_path)");
    ...
    if (!impl_->cpmd->available())
      throw std::runtime_error(
          ...
          "RGPOT_CPMDC_ENGINE or [RGPot] engine_path)");
  } else {
...
void RGPotEngine::force(long N, const double *R, const int *atomicNrs, double *F,
