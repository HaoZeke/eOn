...
* Runtime C ABI (dlopen)

Host =eonclient= / =libeonclib= never links torch. The pot is
=libmetatomic_pot.so=, exporting =eon_mta_*= (see =metatomic_c_abi.h=).
=MetatomicDynPot= loads it via =DynLib= using =EON_POTENTIALS_PATH= /
=LD_LIBRARY_PATH= / =[Potential] potentials_path=.

Build plugin: =-Dwith_metatomic=true= (and =-Dpip_metatomic=true= or system
metatomic/torch).
