 ...
 * @brief A potential class that uses a metatomic model for energy and force
 * calculations.
 *
 * This class loads a pre-trained atomistic model (via metatomic/PyTorch)
 * and uses it to compute the potential energy and corresponding forces on
 * atoms. It uses the `vesin` library to compute neighbor lists required by the
 * model.
 *
 */
...
public:
   ...
   *
   * This is the core method of the potential. It takes the current atomic
   * positions, builds the necessary data structures for metatomic, executes the
   * model to get the potential energy, and uses PyTorch's autograd to compute
   * forces.
   *
   * @param nAtoms Number of atoms.
   * @param positions Flat array of atomic positions (size nAtoms * 3).
   * @param atomicNrs Flat array of atomic numbers (size nAtoms), used for
   * consistency checks.
   ...
   * @param variance Pointer to a double to store the variance of the energy
